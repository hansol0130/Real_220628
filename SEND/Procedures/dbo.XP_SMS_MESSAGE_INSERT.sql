...
------------------------------------------------------------------------------------------------------------------
   2018-06-12		김성호			최초생성
   2018-06-27		정지용			em_tran => tran_etc2 = 'S' 디폴트로 입력 ( 업체 요청 )
   2018-07-12		김성호			result_seq 필수 값 등록
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[XP_SMS_MESSAGE_INSERT]
	@TRAN_PHONE		VARCHAR(15),		-- [수신자휴대폰번호] : 숫자만 입력 예) 01023456789
	@TRAN_CALLBACK	VARCHAR(15),		-- [사전등록된 발신자 전화번호]
	@TRAN_MSG		VARCHAR(2000),		-- [발송메세지]
...
BEGIN
	...
	DECLARE
		...
		@TRAN_TYPE		INT = 4,				-- [4: SMS 전송, 6: LMS/MMS 전송]
		@SERVICE_NO		INT	= 1,				-- [서비스번호] : [1:고객사SMS, 2:고객사LMS]
		@SN				VARCHAR(100),			-- [발송요청ID]
		@TODAY			VARCHAR(8)

	 SET @TODAY = CONVERT(VARCHAR(8), @TRAN_DATE, 112)
	...
	-- 메세지 본문 등록
	INSERT INTO SEND.dbo.em_tran (tran_phone, tran_callback, tran_status, tran_date, tran_msg, tran_type, tran_etc2, tran_etc3, tran_etc4, SERVICE_NO, RESULT_SEQ, REQ_DEPT_ID, REQ_USER_ID)
	...
	VALUES (
		...
		@TRAN_DATE,			-- [발송요청일시]
		CONVERT(VARCHAR(255), @TRAN_MSG),			-- [발송메세지]
	...
	BEGIN
		-- 메세지 마스터 등록
		INSERT INTO SEND.dbo.ALT_MESSAGE_MASTER (SN, TRAN_SN, RES_CODE, CUS_NO, CUS_NAME, AGT_CODE, EMP_SEQ)
		VALUES (@SN, @TRAN_PR, @RES_CODE, @CUS_NO, @CUS_NAME, @AGT_CODE, @EMP_SEQ)
	END
