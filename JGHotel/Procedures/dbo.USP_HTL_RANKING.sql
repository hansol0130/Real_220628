...
GO
CREATE PROC [dbo].[USP_HTL_RANKING]  
...
SET @SQL = @SQL + '  COUNT(CASE WHEN A.LAST_STATUS = ''RMTK''  THEN A.RESV_NO END) AS RMTK_CNT,  ' + CHAR(13)  
SET @SQL = @SQL + '  ISNULL(SUM(CASE WHEN A.LAST_STATUS = ''RMTK''  THEN C.PAY_AMT  ELSE 0 END), 0) AS RMTK_AMT,  ' + CHAR(13)  
SET @SQL = @SQL + '  SUM(CASE WHEN LAST_STATUS IN (''RMXQ'',''RMXX'') THEN 0 WHEN LAST_STATUS IN (''RMTK'') THEN (TOTAL_AMT-SUPP_NET-AIR_COST- COUP_COST -DSC_COST - CARD_COST) END) AS ''RMTK_COM'', ' + CHAR(13)  
SET @SQL = @SQL + '  ISNULL(ROUND(SUM(CASE WHEN A.LAST_STATUS = ''RMTK'' THEN A.COM_AMT ELSE 0 END) / SUM(CASE WHEN A.LAST_STATUS = ''RMTK'' THEN (CASE WHEN A.SUPP_CODE = ''EX'' THEN FLOOR(C.PAY_AMT * A.EX_RATE) ELSE C.PAY_AMT END) ELSE 0 END) * 100, 1), 0) AS COM_RATE ' + CHAR(13)  
...
SET @SQL = @SQL + ' ( ' + CHAR(13)  
SET @SQL = @SQL + '  SELECT ' + CHAR(13)  
SET @SQL = @SQL + '   RESV_NO, SUM(ISNULL(CARD_AMOUNT,0) + ISNULL(CASH_AMOUNT,0)) AS PAY_AMT ' + CHAR(13)  
...
IF(@PARTNER_SHIP <> '') 
SET @SQL = @SQL + ' AND A.CHANNEL = ''' + @PARTNER_SHIP + ''' ' + CHAR(13)   

SET @SQL = @SQL + ' GROUP BY A.HOTEL_NAME, B.CITY_NAME, A.SUPP_CODE ' + CHAR(13)  
