...
GO
CREATE PROC [dbo].[UPS_HTL_SALE_MONTH_BY_CITY3]


/*  
UPS_HTL_SALE_MONTH_BY_CITY3 'R', '2015-02-01', '2015-02-28', '' ,'179900,179897'
*/  
...
SET @SQL = @SQL + '  CONVERT(VARCHAR(10), ' + @T_DATE + ', 121) AS RES_MONTH, ' + CHAR(13)  
SET @SQL = @SQL + '  B.CITY_CODE, ' + CHAR(13)  
SET @SQL = @SQL + '  B.CITY_NAME, ' + CHAR(13)  
SET @SQL = @SQL + '  COUNT(*) AS RES_CNT, ' + CHAR(13)  
...
SET @SQL = @SQL + '  ROUND(SUM(CASE WHEN A.LAST_STATUS = ''RMTK'' THEN A.COM_AMT ELSE 0 END) / SUM(CASE WHEN A.LAST_STATUS = ''RMTK'' THEN (CASE WHEN A.SUPP_CODE = ''EX'' AND MCPN_CODE NOT IN (''VG0000'') THEN FLOOR(C.PAY_AMT * EX_RATE) ELSE C.PAY_AMT END) ELSE 0 END) * 100, 1) AS COM_RATE ' + CHAR(13)  
...
SET @SQL = @SQL + ' WHERE B.CITY_CODE IN (' + @CITY_CODE + ') AND ' + @T_DATE + ' >= ''' + @DATE_FROM + ''' AND ' + @T_DATE + ' < CONVERT(DATETIME, ''' + @DATE_TO + ''') + 1 ' + CHAR(13)  
IF(@PARTNER_SHIP <> '') SET @SQL = @SQL + ' AND A.MCPN_CODE = ''' + @PARTNER_SHIP + ''''  + CHAR(13) 
SET @SQL = @SQL + ' AND A.HOTEL_CODE NOT LIKE ''OF%'' ' + CHAR(13)
SET @SQL = @SQL + ' GROUP BY CONVERT(VARCHAR(10), ' + @T_DATE + ', 121), B.CITY_CODE, B.CITY_NAME ' + CHAR(13)  
SET @SQL = @SQL + ' ORDER BY ''RES_MONTH''' + CHAR(13)  
