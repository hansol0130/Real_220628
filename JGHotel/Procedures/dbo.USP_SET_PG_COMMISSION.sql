...
GO
CREATE PROC [dbo].[USP_SET_PG_COMMISSION]
    
/*    
USP_SET_PG_COMMISSION '1034'    
*/    
...
IF (@SUPP_CODE<>'EX' AND @LAST_STATUS<>'RMXQ' AND @LAST_STATUS<>'RMXX')    
BEGIN     
    
 SELECT @CARD_COST = ISNULL(SUM(DBO.FN_GET_PAY_COMMITION(CARD_AMOUNT+CASH_AMOUNT, PAY_TYPE)),0)
 FROM HTL_RESV_PAY    
 WHERE RESV_NO=@RESV_NO AND USE_YN='Y'    
 AND NOT (PAY_TYPE='VTAC' AND PAY_STATUS='RMXX')
