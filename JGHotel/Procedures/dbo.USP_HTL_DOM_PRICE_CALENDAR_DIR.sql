...
GO
CREATE PROC [dbo].[USP_HTL_DOM_PRICE_CALENDAR_DIR]  
  
/*  
USP_HTL_DOM_PRICE_CALENDAR_DIR '', '', ''
*/  
...
ISNULL(
	CASE
	  WHEN AVAIL_TYPE = 'RQ' AND REQ_REMAIN = 0 THEN 'RX'
	  WHEN AVAIL_TYPE = 'RL' AND REQ_REMAIN = 0 THEN 'RX'
	  WHEN AVAIL_TYPE = 'CF' AND (HARD_REMAIN + SOFT_REMAIN) = 0 THEN 'RX'
	END
...
FROM #TMP_TABLE A  
LEFT JOIN HTL_DOM_INFO_PRICE_DIR B ON B.HOTEL_CODE=@HOTEL_CODE AND B.ROOM_CODE=@ROOM_CODE AND B.CHECK_IN_DATE=A.CAL_DATE  
