...
- 2014-11-19 곽병삼 CTI DB에 SP 내용(SP_CUS_RCV_AGREE_INSERT) Copy
- 2015-09-04 정지용	휴면계정으로 인해 회원테이블 업데이트 분기
===================================================================================*/
CREATE PROCEDURE [cti].[SP_CTI_CUS_RCV_AGREE_INSERT]
	...
	SELECT TOP 1 @RCV_NO = RCV_NO	FROM ( 
		SELECT RCV_NO ,
			...
			CASE WHEN (NOR_TEL1 = @NOR_TEL1 AND NOR_TEL2 = @NOR_TEL2 AND NOR_TEL3 = @NOR_TEL3) THEN SMS_AGREE_DATE 
				WHEN EMAIL = @EMAIL THEN  EMAIL_AGREE_DATE 
			END AS AGREE_DATE 
		FROM Diablo.dbo.CUS_RCV_AGREE WITH(NOLOCK)
		WHERE CUS_NAME = @CUS_NAME 
		AND ((NOR_TEL1 = @NOR_TEL1 AND NOR_TEL2 = @NOR_TEL2 AND NOR_TEL3 = @NOR_TEL3) 
			OR EMAIL = @EMAIL )
	) AS TBL
...
BEGIN
	...
	BEGIN
		-- 이메일주소만 같을때
		SELECT TOP 1 @RCV_NO = RCV_NO FROM Diablo.dbo.CUS_RCV_AGREE WITH(NOLOCK)
		WHERE EMAIL = @EMAIL
...
BEGIN
	...
	INSERT INTO Diablo.dbo.CUS_RCV_AGREE(CUS_NO,CUS_ID,CUS_NAME,BIRTH_DAY,GENDER,EMAIL,NOR_TEL1,NOR_TEL2,NOR_TEL3,
	...
	-- *동의정보 이력 입력 
	INSERT INTO Diablo.dbo.CUS_RCV_AGREE_HISTORY (RCV_NO,CUS_NO,CUS_ID,CUS_NAME,BIRTH_DAY,GENDER,EMAIL,NOR_TEL1,NOR_TEL2,NOR_TEL3,
...
BEGIN 
	-- *동의 정보 갱신
	-- 각 변수에 정보가 있는것만 갱신
	UPDATE Diablo.dbo.CUS_RCV_AGREE 
