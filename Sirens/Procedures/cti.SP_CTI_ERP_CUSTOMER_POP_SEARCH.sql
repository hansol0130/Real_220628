...
■ EXEC						: 
	...
	exec CTI.SP_CTI_ERP_CUSTOMER_POP_SEARCH  '','최현숙','1959-05-01','','','',''
...
------------------------------------------------------------------------------------------------------------------
   ...
   2022-05-09		이호철			탈퇴회원 검색 제외 
================================================================================================================*/ 

CREATE PROCEDURE [cti].[SP_CTI_ERP_CUSTOMER_POP_SEARCH]
--DECLARE
	@CUS_ID		VARCHAR(20),
	@CUS_NAME	VARCHAR(20),
...
SELECT	TOP 200
	...
	(SELECT COUNT(1) FROM CTI.CTI_CONSULT WITH(NOLOCK) WHERE CONSULT_SEQ >= CONVERT(CHAR(8),DATEADD(M,-1,GETDATE()),112) + ''000000'' AND CUS_NO = A.CUS_NO  AND SAVE_TYPE <> ''0'' ) AS CUS_CONSULT_CNT
FROM (
...
LEFT JOIN DIABLO.DBO.CUS_VIP_HISTORY CVH WITH(NOLOCK) ON Z.CUS_NO = CVH.CUS_NO AND CVH.VIP_YEAR = YEAR(GETDATE())
LEFT JOIN (
