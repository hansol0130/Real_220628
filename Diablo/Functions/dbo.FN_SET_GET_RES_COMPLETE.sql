...
-- Description:	예약코드의 수익현황을 가져온다.
-- 로직의 변경이 생긴다면 FN_SET_GET_PRO_COMPLETE 도 변경해야 한다
-- =============================================
CREATE FUNCTION [dbo].[FN_SET_GET_RES_COMPLETE]
(	
	@RES_CODE VARCHAR(20)
)
RETURNS 
...
BEGIN
	...
	-- 전체 인원수를 구한다. (유아를 제외한)
	SELECT @PERSON_COUNT = COUNT(*) FROM SET_CUSTOMER A  WITH(NOLOCK) 
	INNER JOIN RES_CUSTOMER_DAMO B  WITH(NOLOCK) ON B.RES_CODE= A.RES_CODE AND B.SEQ_NO = A.RES_SEQ_NO
	-- 이동과 취소는 포함시키지 않는다.
