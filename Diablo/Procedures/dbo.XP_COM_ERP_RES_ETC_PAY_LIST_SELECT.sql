...
/*================================================================================================================
■ USP_NAME					: XP_COM_ERP_RES_ETC_PAY_LIST_SELECT
■ DESCRIPTION				: BTMS ERP 기타 예약 미수현황 조회
■ INPUT PARAMETER			: 
■ OUTPUT PARAMETER			: 

	EXEC XP_COM_ERP_RES_ETC_PAY_LIST_SELECT NULL, NULL, 1, '2016-04-01', '2016-06-01', NULL, NULL, NULL, NULL, NULL, NULL

	exec XP_COM_ERP_RES_ETC_PAY_LIST_SELECT @RES_STATE=2,@AGT_CODE='',@SEARCH_DATE_TYPE=3,@START_DATE='2016-05-03 00:00:00',@END_DATE='2016-07-21 00:00:00',@PRO_DETAIL_TYPE=10,@SET_STATE=2,@PAY_STATE=0,@PAY_LATER_COM_YN=NULL,@PAY_LATER_YN=NULL,@EMP_CODE=''
	exec XP_COM_ERP_RES_AIR_PAY_LIST_SELECT @RES_STATE=10,@AGT_CODE='',@SEARCH_DATE_TYPE=1,@START_DATE='2016-05-03 00:00:00',@END_DATE='2016-06-30 00:00:00',@AIR_GDS=0,@SET_STATE=0,@PAY_STATE=9,@PAY_LATER_COM_YN=NULL,@PAY_LATER_YN=NULL,@EMP_CODE=''
...
------------------------------------------------------------------------------------------------------------------
   2016-04-27		김성호			최초생성
   2016-05-03		박형만			전체 조건 추가 , 기타예약 조건추가 
...
================================================================================================================*/ 
CREATE PROC [dbo].[XP_COM_ERP_RES_ETC_PAY_LIST_SELECT]
	...
	@START_DATE			DATETIME,
	@END_DATE			DATETIME,
	@PRO_DETAIL_TYPE	INT,				/* public enum BizTripProductTypeEnum { 항공 = 2, 호텔 = 3, 렌트카 = 4, 비자 = 5, 기타 = 9 } */
...
AND BB.PAY_DATE < DATEADD(D,1,@END_DATE) 
	...
	IF @PRO_DETAIL_TYPE IS NOT NULL AND  @PRO_DETAIL_TYPE <> 10  
	BEGIN
		SET @WHERE1 = @WHERE1 + ' AND B.PRO_DETAIL_TYPE = @PRO_DETAIL_TYPE'
	END
	ELSE 
	BEGIN
		SET @WHERE1 = @WHERE1 + ' AND B.PRO_DETAIL_TYPE IN (4,5,9) ' --기타예약
	END 
	...
	BEGIN
		SET @WHERE1 = @WHERE1 + ' AND D.PAY_LATER_YN = ''Y'''
	END
	...
	SET @SQLSTRING = @SQLSTRING + CONVERT(NVARCHAR(MAX), N'
		...
		(
			...
			LEFT JOIN AGT_MASTER D WITH(NOLOCK) ON A.SALE_COM_CODE = D.AGT_CODE
			' + @WHERE1 + N'
			GROUP BY A.RES_CODE, C.SET_STATE
		)
		, PRICE_LIST AS
		(
		...
		SELECT D.PRO_DETAIL_TYPE, Z.RES_CODE, A.RES_STATE, A.DEP_DATE, A.ARR_DATE, A.RES_NAME, B.AGT_CODE, B.AGT_NAME AS [SALE_COM_NAME], A.NEW_CODE, C.KOR_NAME AS [NEW_NAME]
			, Y.TOTAL_PRICE, Y.PAY_PRICE, Y.REST_PRICE, Y.CHG_PRICE, Y.PAY_STATE, ISNULL(Z.SET_STATE, 9) AS [SET_STATE], X.RES_CUS_COUNT
		...
		INNER JOIN PRICE_LIST Y ON Z.RES_CODE = Y.RES_CODE
		INNER JOIN CUS_LIST X ON Z.RES_CODE = X.RES_CODE
		INNER JOIN COM_BIZTRIP_DETAIL A ON Z.RES_CODE = A.RES_CODE
		' + @WHERE2 + N'
