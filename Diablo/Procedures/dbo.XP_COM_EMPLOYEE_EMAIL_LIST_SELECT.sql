...
GO
/*================================================================================================================
■ USP_NAME					: XP_COM_EMPLOYEE_EMAIL_LIST_SELECT
■ DESCRIPTION				: BTMS 거래처 직원 EMAIL 수신 리스트
■ INPUT PARAMETER			: 
...
■ EXEC						: 
	...
	DECLARE @TOTAL INT
	EXEC DBO.XP_COM_EMPLOYEE_EMAIL_LIST_SELECT 1, 10, @TOTAL OUTPUT, 'AgentCode=?EmpSeq=&', 1
...
================================================================================================================*/ 
CREATE PROC [dbo].[XP_COM_EMPLOYEE_EMAIL_LIST_SELECT]
	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	FROM COM_RECEIVE_EMAIL A WITH(NOLOCK)
	...
	-- 리스트 조회
	SELECT A.AGT_CODE, A.EMAIL_SEQ, A.MAIL_TYPE, A.EMP_SEQ, A.RCV_EMAIL, A.BODY, A.RCV_STATE, A.RCV_DATE, A.CFM_DATE, SND_EMAIL, SND_EMP_CODE, B.KOR_NAME AS SND_EMP_NAME, RCV_NAME, TITLE
	FROM COM_RECEIVE_EMAIL A WITH(NOLOCK)
