...
/*================================================================================================================
■ USP_NAME					: ZP_WEB_PKG_MASTER_MONTH_DETAIL_LIST_SELECT
...
■ EXEC						: 
	...
	exec ZP_WEB_PKG_MASTER_MONTH_DETAIL_LIST_SELECT @MASTER_CODE, @START_DATE, @END_DATE, @ORDER_BY
■ MEMO						: 
------------------------------------------------------------------------------------------------------------------
■ CHANGE HISTORY                   
------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------
   ...
   2020-02-18	    임원묵			XP_WEB_PKG_MASTER_MONTH_DETAIL_LIST_SELECT -> ZP_WEB_PKG_MASTER_MONTH_DETAIL_LIST_SELECT 로 복사 생성
   2020-08-18		EHD(김영민)		당일행사에대해 좌석출발날짜/도착날짜 null(DEP_DEP_DATE/ARR_ARR_DATE) => 행사날짜로 대체
================================================================================================================*/ 

CREATE  PROCEDURE [dbo].[ZP_WEB_PKG_MASTER_MONTH_DETAIL_LIST_SELECT]
(
...
BEGIN
	...
	SELECT * FROM (
		SELECT @START_DATE AS [BASIC_DATE], A.PRO_CODE, A.DEP_DATE, A.ARR_DATE, A.PRO_NAME, A.MAX_COUNT, A.MIN_COUNT, A.FAKE_COUNT
			...
			, C.DEP_TRANS_NUMBER AS [TRANS_NUMBER]-- 2019-11-06 추가
			, (SELECT COUNT(*) FROM PKG_DETAIL_OPTION WITH(NOLOCK) WHERE PRO_CODE = Z.PRO_CODE) AS OPT_CNT
			, (SELECT COUNT(*) FROM PKG_DETAIL_SHOPPING WITH(NOLOCK) WHERE PRO_CODE = Z.PRO_CODE) AS SHOP_CNT
		FROM LIST Z
