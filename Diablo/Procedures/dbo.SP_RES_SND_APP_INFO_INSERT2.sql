...
------------------------------------------------------------------------------------------------------------------
   2018-07-24		김성호			최초생성
   2021-04-05		오준혁			롯데면세점 프로모션 상품 전용 알림톡 등록
================================================================================================================*/ 
CREATE PROC [dbo].[SP_RES_SND_APP_INFO_INSERT2]
...
BEGIN
	...
	DECLARE @END_DATE      DATETIME
	       ,@TMPL_CD       VARCHAR(20) = 'SYS_AT_0053'	-- 사용 템플릿 번호
	       ,@CONTS_TXT     VARCHAR(4000)
	       ,@REQ_DTM       VARCHAR(14)
	       ,@CONTS_TXT2    VARCHAR(4000) 
    ...
    /*
    롯데면세점 프로모션 상품 전용 알림톡 등록
    상품코드: HTF044, HTF045
    프로모션 기간: 2021.04.14 ~ 05.31 (예약시)
    출발기간: 2021.07.21 ~ 09.18 (출발 3일전)
                        
    SYS_AT_0135 알림톡 템플릿으로 전송 (롯데면세점_출발전혜택안내)
    */
	DECLARE @SDate             DATE = '2021-07-21'
	       ,@NDate             DATE = CONVERT(DATE ,@CHECK_DATE)
	       ,@EDate             DATE = '2021-09-19' -- 1일 더한다
	       ,@PromoCode         VARCHAR(10)
	 
	 
	
	SELECT @CONTS_TXT2 = A.CONTS_TXT
	FROM   SEND.dbo.NVMOBILECONTENTS A WITH(NOLOCK)
	WHERE  A.KAKAO_TMPL_CD = 'SYS_AT_0135'

	SET @CONTS_TXT2 = REPLACE(@CONTS_TXT2, '#{롯데면세점url}', 'http://lotte.com')
	SET @CONTS_TXT2 = REPLACE(@CONTS_TXT2, '#{코드등록기간}', '기간')
	...
	(
	    ...
	    SELECT A.RES_CODE
	          ,B.SEQ_NO
	          ,B.CUS_NO
	          ,(B.NOR_TEL1 + B.NOR_TEL2 + B.NOR_TEL3) AS [PHONE_NUM]
	          ,B.CUS_NAME
	          ,A.MASTER_CODE
	    FROM   RES_MASTER_damo A WITH(NOLOCK)
	           INNER LOOP JOIN RES_CUSTOMER_damo B WITH(NOLOCK)
	    ...
	    WHERE  A.DEP_DATE >= CONVERT(DATE ,@CHECK_DATE)
	           AND A.DEP_DATE < @END_DATE
	           AND A.RES_STATE < 7
	           AND B.RES_STATE = 0
	           AND A.PRO_TYPE = 1
	           AND A.PRO_CODE LIKE '[^K]%'
	           AND D.SIGN_CODE LIKE '[^K]'
	           AND D.ATT_CODE IN ('P' ,'R' ,'W' ,'G') --[마스터 대표속성] 패키지,실시간항공,허니문,골프
	           AND C.TRANSFER_TYPE = 1 -- [교통편] 항공편
	           AND B.NOR_TEL1 IS NOT NULL
	           AND B.NOR_TEL2 IS NOT NULL
	           AND B.NOR_TEL3 IS NOT NULL
	    UNION ALL
	    SELECT A.RES_CODE
	          ,B.SEQ_NO
	          ,B.CUS_NO
	...
	(
	    SELECT A.RES_CODE
	          ...
	          ,A.PHONE_NUM
	          ,(CASE 
	                 WHEN MIN(A.CUS_NO) > 1 THEN MIN(A.CUS_NO)
	                 ELSE NULL
	            END) AS [CUS_NO]
	          ,MIN(A.CUS_NAME) AS 'CUS_NAME'
	          ,MIN(A.MASTER_CODE) AS 'MASTER_CODE'
	    FROM   RES_LIST A
	...
	FROM   SEND_LIST A
	       LEFT JOIN (
	                ...
	                FROM   SEND.[dbo].[ALT_MESSAGE_MASTER] AA WITH(NOLOCK)
	                       INNER JOIN SEND.DBO.MZSENDLOG BB WITH(NOLOCK)
	                            ON  AA.SN = BB.SN
	                                AND BB.TMPL_CD = @TMPL_CD
	                WHERE  AA.RES_CODE IN (SELECT DISTINCT RES_CODE
	                                       FROM   RES_LIST)
	                GROUP BY
	                       AA.RES_CODE
	                      ,BB.PHONE_NUM
	...
	OPEN USER_CURSOR
	FETCH NEXT FROM USER_CURSOR INTO @RES_CODE, @SEQ_NO, @PHONE_NUM, @CUS_NO, @CUS_NAME, @MASTER_CODE
	...
	BEGIN
		...
		출발기간: 2021.07.21 ~ 09.18 (출발 3일전)
		*/
		IF (@MASTER_CODE = 'HTF044' OR @MASTER_CODE = 'HTF045')
		BEGIN
			
			-- 출발기간
			IF (@NDate >= @SDate AND @NDate < @EDate)
			BEGIN
				
				--EXEC dbo.ZP_EVENT_LOTTE_PROMO_CODE_SELECT @CUS_NO, @RES_CODE, @PromoCode OUTPUT
				
				SET @CONTS_TXT2 = REPLACE(@CONTS_TXT2, '#{NAME}', @CUS_NAME)
				SET @CONTS_TXT2 = REPLACE(@CONTS_TXT2, '#{난수번호}', @PromoCode)
			
				-- 알림톡 발송
				--EXEC SEND.dbo.XP_ALT_MESSAGE_INSERT @SENDER_KEY, @PHONE_NUM, 'SYS_AT_0135', @CONTS_TXT2, @REQ_DTM, '롯데면세점_출발전혜택안내', '02-2188-4000', '529', '9999999', 'Y', 'R', @RES_CODE, @CUS_NO, NULL, NULL, NULL
