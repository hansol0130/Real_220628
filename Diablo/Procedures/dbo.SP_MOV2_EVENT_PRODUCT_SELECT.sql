...
GO
/*================================================================================================================          
■ USP_NAME     : [SP_MOV2_EVENT_PRODUCT_SELECT]          
■ DESCRIPTION    : 이벤트 상세정보 조회          
■ INPUT PARAMETER   : @SNS_COMPANY, @SNS_ID          
■ EXEC      :           
    -- EXEC SP_MOV2_EVENT_PRODUCT_SELECT 6635, 0, 0          
...
------------------------------------------------------------------------------------------------------------------          
   ...
   2019-12-11 임검제 리뉴얼 관련 select 항목 추가     
   2020-01-02 임검제 행사 출발 시간 추가 
================================================================================================================*/           
CREATE PROCEDURE [dbo].[SP_MOV2_EVENT_PRODUCT_SELECT]          
...
BEGIN          
 ...
 -- interfering with SELECT statements.          
  ...
  WITH LIST AS (          
   SELECT          
    A.EVT_SEQ, A.EVT_DATA_SEQ, A.CATEGORY, A.SUB_CATEGORY, A.MASTER_CODE, A.PRO_CODE, A.PRICE_SEQ, A.CODE_TYPE, A.SHOW_YN, A.SORT_NUM            
    , 'M' AS [FLAG], B.SIGN_CODE, B.MASTER_NAME AS [NAME], B.MASTER_CODE AS [CODE], B.PKG_COMMENT          
    ...
    , C.FILE_CODE, C.REGION_CODE, C.NATION_CODE, C.STATE_CODE, C.CITY_CODE, C.KOR_NAME, C.ENG_NAME, C.FILE_TYPE, C.FILE_NAME, C.EXTENSION_NAME, C.FILE_SIZE          
    , C.FILE_NAME_S, C.FILE_NAME_M, C.FILE_NAME_L, C.FILE_REMARK, C.FILE_TAG, C.RESOLUTION, C.COPYRIGHT, C.COPYRIGHT_REMARK          
    , B.BRANCH_CODE, B.ATT_CODE, B.BRAND_TYPE          
    , (SELECT TOP 1 PUB_VALUE FROM COD_PUBLIC AA WITH(NOLOCK) WHERE PUB_TYPE = 'PKG.ATTRIBUTE' AND AA.PUB_CODE = B.ATT_CODE) AS [ATT_NAME]          
    ...
    , B.EVENT_PRO_CODE          
	 ...
	 -- 2019 리뉴얼 추가         
	 , STUFF((SELECT (','+UPPER(SEARCH_VALUE)) as [text()] FROM PKG_MASTER_SUMMARY WHERE MASTER_CODE = B.MASTER_CODE AND SEARCH_TYPE = 'W' FOR XML PATH('')), 1, 1, '') AS WEEK_DAY -- 출발일        
	 , DBO.FN_GET_MASTER_SUMMARY(B.MASTER_CODE, 'D') AS TOUR_DAY        
	 , (SELECT COUNT(DISTINCT SEARCH_VALUE) FROM PKG_MASTER_SUMMARY WHERE B.MASTER_CODE = MASTER_CODE AND SEARCH_TYPE = 'D') AS TOUR_DAY_COUNT -- 여행일정 ~부터 표시를 위해서 count를 확인함.    
   ...
   SELECT           
    A.EVT_SEQ, A.EVT_DATA_SEQ, A.CATEGORY, A.SUB_CATEGORY, A.MASTER_CODE, A.PRO_CODE, A.PRICE_SEQ, A.CODE_TYPE, A.SHOW_YN, A.SORT_NUM          
    , 'P' AS [FLAG], D.SIGN_CODE, B.PRO_NAME AS [NAME], B.PRO_CODE AS [CODE], D.PKG_COMMENT          
    , C.PKG_INCLUDE, B.DEP_DATE, DBO.XN_PRO_DETAIL_SALE_PRICE_CUTTING(A.PRO_CODE, A.PRICE_SEQ) AS [PRICE]          
    , E.FILE_CODE, E.REGION_CODE, E.NATION_CODE, E.STATE_CODE, E.CITY_CODE, E.KOR_NAME, E.ENG_NAME, E.FILE_TYPE, E.FILE_NAME, E.EXTENSION_NAME, E.FILE_SIZE          
    , E.FILE_NAME_S, E.FILE_NAME_M, E.FILE_NAME_L, E.FILE_REMARK, E.FILE_TAG, E.RESOLUTION, E.COPYRIGHT, E.COPYRIGHT_REMARK          
    , D.BRANCH_CODE, D.ATT_CODE, D.BRAND_TYPE          
    , (SELECT TOP 1 PUB_VALUE FROM COD_PUBLIC AA WITH(NOLOCK) WHERE PUB_TYPE = 'PKG.ATTRIBUTE' AND AA.PUB_CODE = D.ATT_CODE) AS [ATT_NAME]          
    ...
    , (SELECT COUNT(*) FROM PUB_EVENT_DATA A2 WITH(NOLOCK) INNER JOIN PUB_EVENT B2 WITH(NOLOCK) ON A2.EVT_SEQ = B2.EVT_SEQ WHERE B2.EVT_YN = 'Y' AND A2.SHOW_YN = 'Y' AND B2.SHOW_YN = 'Y' AND A2.MASTER_CODE = D.MASTER_CODE AND B2.END_DATE >= GETDATE()) AS [EVENT_COUNT]          
    , D.TAG AS [TAG]           
    , D.EVENT_PRO_CODE          
	 ...
	 , B.TOUR_JOURNEY        
	 , D.AIRLINE_CODE        
	 , (CASE WHEN EXISTS(SELECT * FROM PKG_MASTER PM WHERE PM.EVENT_PRO_CODE = A.PRO_CODE)       
		 THEN 1      
		 ELSE 0 END ) AS IS_MASTEREVENT_PRODUCT      
	 , F.DEP_DEP_TIME
      
   FROM PUB_EVENT_DATA A WITH(NOLOCK)          
   INNER JOIN PKG_DETAIL B WITH(NOLOCK) ON A.PRO_CODE = B.PRO_CODE          
   INNER JOIN PRO_TRANS_SEAT F WITH(NOLOCK) ON B.SEAT_CODE = F.SEAT_CODE
  ...
  SELECT          
   ...
   , (          
    ...
    WHERE A1.PRO_CODE = Z.EVENT_PRO_CODE AND A1.DEP_DATE >= GETDATE()          
   ) AS [EVENT_NAME]          
   , (          
 ...
 ORDER BY PRICE_SEQ        
  ...
  FROM LIST Z          
  OUTER APPLY (SELECT TOP 1 * FROM PRO_COMMENT AS COMMENT WHERE Z.MASTER_CODE = COMMENT.MASTER_CODE ORDER BY COMMENT.NEW_DATE DESC, COM_SEQ DESC) AS COMMENT         
  LEFT OUTER JOIN PUB_AIRLINE AS AIRLINE ON AIRLINE.AIRLINE_CODE = Z.AIRLINE_CODE        
