...
-- 2014-10-16	박형만 마스터 코드 조건 추가 - 국내팀요청 
-- 2016-04-12	정지용 도착일 추가 (ARR_DATE) - 재무회계팀 요청
-- =============================================
CREATE PROCEDURE [dbo].[SP_SET_GET_RES_COMPLETE_LIST_4]
(
	@DATE_TYPE	INT,
	@EMP_TYPE	INT,
	@START_DATE	DATETIME,
...
SET @SQLSTRING_PROFIT_TYPE_1 =N'SELECT
	    B.MASTER_CODE, A.PRO_CODE, C.RES_CODE, A.DEP_DATE, A.ARR_DATE, D.TEAM_CODE, 
        C.PROFIT_EMP_CODE,
        C.PROFIT_TEAM_CODE,
        C.PROFIT_TEAM_NAME     
FROM SET_MASTER A
...
BEGIN
SET @SQLSTRING =  @SQLSTRING + ' AND E.MASTER_CODE = @MASTER_CODE ' 
END
...
GROUP BY A.PRO_CODE, B.PRO_NAME, B.DEP_DATE, B.ARR_DATE, C.SIGN_CODE, A.PROFIT_TEAM_NAME
ORDER BY 
...
SET @PARMDEFINITION = N'@DATE_TYPE	INT,	@EMP_TYPE	INT,	@START_DATE	DATETIME,	@END_DATE	DATETIME,	@REGION	CHAR(1),	@NATION_CODE	VARCHAR(30),	@CITY_CODE	VARCHAR(30), @MASTER_CODE	VARCHAR(20), 	@ATTRIBUTE	CHAR(1),	@TEAM_CODE	VARCHAR(3),	@EMP_CODE	VARCHAR(7),	@GROUP	INT,	@ORDER	INT';

PRINT @SQLSTRING_PROFIT_TYPE_2

EXEC SP_EXECUTESQL @SQLSTRING, @PARMDEFINITION, 	@DATE_TYPE,	@EMP_TYPE,	@START_DATE,	@END_DATE,	@REGION,	@NATION_CODE,	@CITY_CODE,	@MASTER_CODE , @ATTRIBUTE,	@TEAM_CODE,	@EMP_CODE,	@GROUP,	@ORDER;
