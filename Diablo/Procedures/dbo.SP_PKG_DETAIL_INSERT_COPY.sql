...
/*================================================================================================================    
■ USP_Name			: SP_PKG_DETAIL_INSERT_COPY
■ Description		: 행사마스터 시즌복사(기간별). MASTER->MASTER정보 & 입력된 정보로 LOOP를 돌며 DETAIL 복사
■ Input Parameter   : 
■ Output Parameter	:                      
■ Output Value		:                     
■ Exec				: 
...
================================================================================================================*/   
CREATE PROCEDURE [dbo].[SP_PKG_DETAIL_INSERT_COPY]
...
BEGIN
	...
	--DECLARE @DEP_TRANS_CODE CHAR(2), @DEP_TRANS_NUMBER CHAR(4)
	--DECLARE @ARR_TRANS_CODE CHAR(2), @ARR_TRANS_NUMBER CHAR(4)
	...
	BEGIN
		...
		BEGIN
			...
			SELECT
				...
				--, 
				--@DIFF_DAY = ISNULL(DATEDIFF(DAY, DEP_DEP_DATE, ARR_ARR_DATE), 0)
	...
	--	SELECT @BIT_CODE = SUBSTRING(PRO_CODE, 14, 10), @NEW_MASTER_CODE = MASTER_CODE
		...
		BEGIN
			...
			BEGIN
				...
				--	AND DEP_DEP_DATE = @START_DATE AND ARR_ARR_DATE = DATEADD(DAY, @DIFF_DAY, @START_DATE))  , 0 ) 
				--END 
				--ELSE
				--BEGIN
				--	SET @NEW_SEAT_CODE = 0
				...
				BEGIN
					BEGIN TRY
						...
						SET @NEW_PRO_CODE = (@MASTER_CODE + '-' + SUBSTRING(CONVERT(VARCHAR(10), @START_DATE, 112), 3, 10) + @BIT_CODE)
						...
						BEGIN
							...
							SELECT
								...
								, @TOUR_NIGHT, @TOUR_DAY, @SEAT_STATUS, @FAKE_COUNT, @MAX_COUNT, @MIN_COUNT, DATEADD(DAY, @LAST_PAY_INTERVAL, @START_DATE)
