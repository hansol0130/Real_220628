...
GO
/*================================================================================================================
■ USP_NAME					: XP_WEB_PKG_MASTER_LIST_SELECT
...
■ EXEC						: 
	...
	exec XP_WEB_PKG_MASTER_LIST_SELECT @page_index, @page_size, @total_count output, @key, @order_by
	SELECT @TOTAL_COUNT
...
================================================================================================================*/ 
CREATE  PROCEDURE [dbo].[XP_WEB_PKG_MASTER_SEARCH_LIST_SELECT]
(
	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	DECLARE @SQLSTRING NVARCHAR(MAX), @SQLSTRING_ADD NVARCHAR(MAX), @PARMDEFINITION NVARCHAR(1000);
...
--			WHEN 1 THEN 'STAR_POINT DESC' -- 'A.REGION_ORDER' --2019-12-11 지역추천 순 --> 평점높은순으로 변경(TO_BE 지역추천 순 사용 안함)
			...
			WHEN 9 THEN ' FAMILY_PERCENT DESC, TOTAL_COUNT DESC'
			WHEN 10 THEN ' MEETING_PERCENT DESC, TOTAL_COUNT DESC'
			WHEN 11 THEN 'COUPLE_PERCENT DESC, TOTAL_COUNT DESC'
			WHEN 12 THEN ' FRIEND_PERCENT DESC, TOTAL_COUNT DESC'
			WHEN 13 THEN ' ALONE_PERCENT DESC, TOTAL_COUNT DESC'
			ELSE 'A.REGION_ORDER'
		END
	)
	...
	(
		SELECT A.MASTER_CODE
			...
			, FAMILY_PERCENT, MEETING_PERCENT, COUPLE_PERCENT, FRIEND_PERCENT, ALONE_PERCENT
		FROM PKG_MASTER A WITH(NOLOCK)
	...
	SELECT	A.MASTER_CODE
		...
		,	B.FILE_CODE, B.REGION_CODE, B.NATION_CODE, B.STATE_CODE, B.CITY_CODE, B.KOR_NAME, B.ENG_NAME, B.FILE_TYPE, B.[FILE_NAME], B.EXTENSION_NAME, B.FILE_SIZE, B.FILE_NAME_S, B.FILE_NAME_M, B.FILE_NAME_L, B.FILE_REMARK, B.FILE_TAG, B.RESOLUTION, B.COPYRIGHT, B.COPYRIGHT_REMARK, B.SHOW_YN, B.NEW_CODE, B.NEW_NAME, ISNULL(B.NEW_DATE, CONVERT(DATETIME, ''1900-01-01'')) AS NEW_DATE, B.EDT_CODE, B.EDT_NAME, ISNULL(B.EDT_DATE, CONVERT(DATETIME, ''1900-01-01'')) AS EDT_DATE
