...
■ Server					: 
■ Database					: DIABLO
■ USP_Name					: XP_WEB_DEV_EVT_SEARCH_LIST_SELECT
...
■ Exec						: 
	...
	declare @total int
	exec [dbo].[ZP_WEB_DEV_EVT_SEARCH_LIST_SELECT] 1, 20, @total output, 'PlanType=A&IsEvent=N&AttrCode=T&EventName=', 0

declare @p5 int
set @p5=0
exec ZP_WEB_DEV_EVT_SEARCH_LIST_SELECT @PAGE_INDEX=1,@PAGE_SIZE=12,@KEY=N'PlanType=M&IsEvent=Y&IsClose=',@ORDER_BY=0,@TOTAL_COUNT=@p5 output
select @p5
...
---------------------------------------------------------------------------------------------------
	2019-08-09 		지니웍스	    기존 로직에서 변경되는 사항이 발생하여 추가(이벤트 시작순, 이벤트 종료순 로직추가)
-------------------------------------------------------------------------------------------------*/ 
CREATE PROCEDURE [dbo].[ZP_WEB_DEV_EVT_SEARCH_LIST_SELECT] 
 ( 
 	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
*/
	...
	SELECT @SORT_STRING = (  
		CASE @ORDER_BY  
			...
			WHEN 2 THEN ' A.BEST_YN DESC, A.START_DATE DESC '
			WHEN 3 THEN ' A.BEST_YN DESC, A.END_DATE '
