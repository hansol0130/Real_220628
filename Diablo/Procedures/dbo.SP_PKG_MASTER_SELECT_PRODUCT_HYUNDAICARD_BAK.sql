...
/*================================================================================================================
■ USP_NAME					: SP_PKG_MASTER_SELECT_PRODUCT_HYUNDAICARD_BAK
■ DESCRIPTION				: 현대카드 상품연동
■ INPUT PARAMETER			: 
■ OUTPUT PARAMETER			: 
■ EXEC						: SP_PKG_MASTER_SELECT_PRODUCT_HYUNDAICARD_BAK 'AFW075'
SP_PKG_MASTER_SELECT_PRODUCT_HYUNDAICARD_BAK ''
■ MEMO						: 
------------------------------------------------------------------------------------------------------------------
■ CHANGE HISTORY                   
------------------------------------------------------------------------------------------------------------------
...
================================================================================================================*/ 
CREATE PROC [dbo].[SP_PKG_MASTER_SELECT_PRODUCT_HYUNDAICARD_BAK]	
...
BEGIN
	...
	*/
	...
	SELECT
		...
		, CASE 
			WHEN Z.BRAND_TYPE IS NOT NULL THEN 'PREMIUM' 
			WHEN Z.ATT_CODE = 'W' THEN 'HONEYMOON' 
			WHEN Z.ATT_CODE = 'F' THEN 'FREE'
			ELSE 'PACKAGE' END AS MAIN_ETC_CODE
		, A.TOUR_DAY
		,E.CITY_CODE AS CUM_CITY_CODE 
		,E.NATION_CODE AS CUM_COUNTRY_CODE 
		...
		, CASE 
			...
			WHEN A.RES_ADD_YN = 'Y' AND (A.MAX_COUNT = 0 OR (A.MAX_COUNT - DBO.FN_PRO_GET_RES_COUNT(A.PRO_CODE) > 0)) AND A.DEP_CFM_YN = 'Y' THEN 'LEVDC' -- 출발확정
