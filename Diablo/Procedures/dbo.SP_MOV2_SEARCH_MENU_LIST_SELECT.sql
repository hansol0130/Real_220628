...
GO
/*================================================================================================================
■ USP_NAME					: SP_MOV2_SEARCH_MENU_LIST_SELECT
■ DESCRIPTION				: 메뉴 설정값 검색
■ INPUT PARAMETER			: 
	@SITE_CODE				: 사이트코드
	@MENU_CODE				: 메뉴코드
	@REAL_YN				: 실서버 유무
...
■ EXEC						: 
	...
	exec SP_MOV2_SEARCH_MENU_LIST_SELECT 'VGT', '101', 'N','홍콩'
...
------------------------------------------------------------------------------------------------------------------
									XP_PUB_MENU_LIST_SELECT 참고로 만든 듯
================================================================================================================*/ 

CREATE  PROCEDURE [dbo].[SP_MOV2_SEARCH_MENU_LIST_SELECT]
(
	@SITE_CODE	VARCHAR(3),
...
BEGIN
	...
	--LEFT JOIN (
	--	SELECT AA.FLAG, AA.DATA, SUM(COUNT) AS COUNT
	--	FROM PUB_EVENT_COUNT AA WITH(NOLOCK)
	--	--WHERE PATINDEX(''%'' + @SITE_CODE + ''%'', AA.PROVIDER) > 0
	--	WHERE CHARINDEX(''5'', PROVIDER) > 0
	--	GROUP BY AA.FLAG, AA.DATA
	--) D ON D.DATA = (CASE D.FLAG WHEN ''A'' THEN B.ATT_CODE WHEN ''R'' THEN B.REGION_CODE END)
	WHERE A.USE_YN = ''Y'' AND C.MENU_NAME like ''%'+@KEYWORD+'%''
