...
/*================================================================================================================
■ USP_NAME					: ZP_CUS_SLEEP_INFO_SND_INSERT
■ DESCRIPTION				: 휴면회원 안내 카카오톡 발송
■ INPUT PARAMETER			: 
■ OUTPUT PARAMETER			: 
■ EXEC						: 

	EXEC DIABLO.DBO.ZP_CUS_SLEEP_INFO_SND_INSERT 'f1e8319931392a3c30bb6fdef415f67ef87aa58c'
...
------------------------------------------------------------------------------------------------------------------
   ...
   2021-01-27		김영민			휴면회원안내 조건 추가(포인트/상담/구매이력), 평생회원제외
================================================================================================================*/ 
CREATE PROC [dbo].[ZP_CUS_SLEEP_INFO_SND_INSERT]
	@SENDER_KEY		VARCHAR(40)
AS 
BEGIN
	
	DECLARE
		@TMPL_CD VARCHAR(20) = 'SYS_AT_0114',		-- 사용 템플릿 번호
		@TODAY DATE = CONVERT(DATE, GETDATE()),		-- 오늘 YYYY-MM-DD 00:00:00	
		@SUBJECT VARCHAR(40) = '휴면예정안내',		-- [LMS 제목]
		...
		@DAY INT,									-- 발송기준 (30: 30일 전, 15: 15일 전)
		@CUS_NO INT,								-- 고객번호
	...
	DECLARE USER_CURSOR CURSOR FOR
		...
		-- 30일 전
		SELECT A.CUS_NO, A.CUS_NAME, A.CUS_ID, (A.NOR_TEL1 + A.NOR_TEL2 + A.NOR_TEL3), 30 AS [DAY] 
		FROM Diablo.dbo.CUS_MEMBER A WITH(NOLOCK)
		WHERE A.LAST_LOGIN_DATE > DATEADD(DAY, -335, @TODAY) AND A.LAST_LOGIN_DATE < DATEADD(DD, 1, DATEADD(DAY, -335, @TODAY))		
		AND A.CUS_NO  NOT IN  --포인트 이력Histroy안봄
		...
		UNION ALL
		-- 15일 전
		SELECT A.CUS_NO, A.CUS_NAME, A.CUS_ID, (A.NOR_TEL1 + A.NOR_TEL2 + A.NOR_TEL3), 15 AS [DAY]
		FROM Diablo.dbo.CUS_MEMBER A WITH(NOLOCK)
		WHERE A.LAST_LOGIN_DATE > DATEADD(DAY, -350, @TODAY) AND A.LAST_LOGIN_DATE < DATEADD(DD, 1, DATEADD(DAY, -350, @TODAY))
		AND A.CUS_NO  NOT IN  --포인트 이력 Histroy안봄
					(SELECT  CUS_NO FROM CUS_POINT  WHERE NEW_DATE  >   DATEADD(DAY, -365, @TODAY)
					AND NEW_DATE  < DATEADD(DD, 1, @TODAY)  AND POINT_NO  NOT IN  (
					SELECT  CUS_POINT.POINT_NO
					FROM CUS_POINT  WHERE POINT_TYPE  = '2' AND ACC_USE_TYPE = '2')
	...
	FETCH NEXT FROM USER_CURSOR	INTO @CUS_NO, @CUS_NAME, @CUS_ID, @NOR_TEL, @DAY
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @CONTS_TXT = REPLACE(REPLACE(REPLACE(A.CONTS_TXT
				, '#{NAME}', @CUS_NAME)
				, '#{아이디}', @CUS_ID)
				, '#{휴면예정일}', CONVERT(VARCHAR(10), DATEADD(DAY, @DAY, @TODAY), 120))	-- 30전, 15일 전 계산
