...
-- Description:	호텔 판매현황 SP 
-- SP_RES_HOTEL_SALE_LIST_SUM_BY_SUP 0 , '2011-06-01','2011-08-10','320','','','','4' 
-- 수정내역 
-- 2011-10-24 호텔 NET_PRICE 취소이동환불은 0 원으로 
-- 2011-10-24 CXL_COUNT 취소건수 추가  
-- 2015-01-15 신규호텔검색되도록 수정
-- 2015-04-07 호텔판매현황 국내호텔D,해외호텔O 구분 
-- =============================================

CREATE PROCEDURE [dbo].[SP_RES_HOTEL_SALE_LIST_SUM_BY_SUP]
	@SEARCH_TYPE INT,
...
---------------------------------------------------------------------------
	...
	SET @ORDER_BY = ' ORDER BY A.SUP_CODE DESC   ' 

	SET @SQLSTRING = N'
SELECT 
	A.SUP_CODE
	, SUM(A.RES_COUNT) AS RES_COUNT
	, SUM(A.CXL_COUNT) AS CXL_COUNT
	, SUM(A.PAY_PRICE) AS [PAY_PRICE]
	, SUM(A.TOTAL_PRICE) AS [TOTAL_PRICE]
	, SUM(A.NET_PRICE) AS [NET_PRICE]
	, SUM(A.SALE_PROFIT) AS [SALE_PROFIT]
FROM (
	SELECT 
		--(CASE C.RES_STATE 	WHEN 8 THEN 998	WHEN 9 THEN 999	ELSE 1	END) AS [NO]
		(CASE WHEN C.RES_STATE NOT IN (7,8,9) THEN 1 ELSE 0	END) AS RES_COUNT ,
		(CASE WHEN C.RES_STATE IN (7,8,9) THEN 1 ELSE 0	END) AS CXL_COUNT ,
