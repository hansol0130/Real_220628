...
■ Server     :         
■ Database     : DIABLO        
■ USP_Name     : XP_WEB_EVT_SEARCH_LIST_SELECT        
...
■ Exec      :         
 ...
 declare @total int        
 exec [dbo].[XP_WEB_EVT_SEARCH_LIST_SELECT] 1, 20, @total output, 'PlanType=A&IsEvent=N&AttrCode=T&EventName=', 0        
        
declare @p5 int        
set @p5=0        
exec XP_WEB_EVT_SEARCH_LIST_SELECT @PAGE_INDEX=1,@PAGE_SIZE=12,@KEY=N'PlanType=M&IsEvent=Y&IsClose=',@ORDER_BY=0,@TOTAL_COUNT=@p5 output        
select @p5        
...
-------------------------------------------------------------------------------------------------*/         
CREATE PROCEDURE [dbo].[XP_WEB_EVT_SEARCH_LIST_SELECT] 
...
BEGIN
	...
	*/        
	...
	IF ISNULL(@KEYWORD ,'') <> ''
	BEGIN
	    SET @WHERE = @WHERE + ' AND (A.EVT_NAME LIKE ''%'' + @KEYWORD + ''%'' OR EVT_SEQ IN (SELECT EVT_SEQ FROM PUB_EVENT_DATA WITH(NOLOCK) WHERE MASTER_CODE = @KEYWORD))'
	END 
...
(        
 SELECT        
  ...
  STUFF((         
  SELECT ('', ''+UPPER(MASTER_CODE)) as [text()] FROM PUB_EVENT_DATA WITH(NOLOCK) WHERE EVT_SEQ = A.EVT_SEQ         
  FOR XML PATH('''')), 1, 1, '''') AS DATA_MASTER,        
  STUFF((         
  SELECT ('',''+PUB_VALUE) as [text()] FROM COD_PUBLIC WITH(NOLOCK) WHERE PUB_TYPE = ''EVENT.REGION'' AND  charindex( PUB_CODE ,A.SIGN_CODE  ) > 0          
