...
/*================================================================================================================
■ USP_NAME					: SP_MOV2_CURATION_LIST_PAGING_SELECT
■ DESCRIPTION				: 검색_큐레이션정보_목록_페이징
■ INPUT PARAMETER			: 
■ EXEC						: 
    -- EXEC SP_MOV2_CURATION_LIST_PAGING_SELECT '2017-09-28', '2017-10-05', 1, 10, '', '', '', '', '', ''
...
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[SP_MOV2_CURATION_LIST_PAGING_SELECT]
...
BEGIN
	...
	SET @SQLSTRING = N'
		SELECT * FROM
		(
			SELECT A.*,ROW_NUMBER() OVER (ORDER BY A.CUR_ORDER) AS NUM
			FROM (
				SELECT A.*, B.KOR_NAME
