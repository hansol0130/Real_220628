...
---------------------------------------------------------------------------------------------------
  ...
  2020-07-20			김영민 			  FORCESEEK 추가(인덱스)
-------------------------------------------------------------------------------------------------*/ 
CREATE PROCEDURE [dbo].[XP_PKG_TC_DETAIL_SELECT_LIST]
(        
	@TC_YN		CHAR(1),
	@NOW_MONTH	VARCHAR(10),
	@TEAM_CODE	VARCHAR(4),
...
BEGIN
	...
	BEGIN 
			...
			ELSE
				BEGIN
					SET @WHERE = ' AND AA.NEW_CODE = ''' + @EMP_CODE + ''' '
				END
	...
	BEGIN 
		SET @SQLSTRING = N'				
				WITH LIST AS(
					SELECT AA.PRO_CODE, COUNT(AA.PRO_CODE) AS CUSTOMER_COUNT  FROM PKG_DETAIL AA WITH(NOLOCK)
						LEFT OUTER JOIN dbo.PRO_TRANS_SEAT B WITH(NOLOCK) ON AA.SEAT_CODE = B.SEAT_CODE
						LEFT JOIN RES_MASTER_damo C WITH(NOLOCK) ON AA.PRO_CODE = C.PRO_CODE  
						LEFT JOIN RES_CUSTOMER_damo D WITH(NOLOCK) ON C.RES_CODE = D.RES_CODE  
					WHERE 1=1
						AND AA.TC_YN = ''Y''
						AND AA.DEP_DATE >= ''' + @NOW_MONTH + '''
						AND AA.DEP_DATE <= DATEADD(ms,-3,DATEADD(mm, DATEDIFF(m,0, CONVERT(DATETIME, ''' + @NOW_MONTH + ''') )+1, 0))  
						'+ @WHERE +'	
						AND C.RES_STATE < 5
					GROUP BY AA.PRO_CODE
				)
				SELECT 
					A.PRO_CODE, CUSTOMER_COUNT, A.PRO_NAME, A.TC_CODE, isnull(A.TC_NAME, '''') AS TC_NAME, A.TC_ASSIGN_YN
					...
					, DATEDIFF(DAY, (CASE WHEN A.SEAT_CODE = 0 THEN A.DEP_DATE ELSE S.DEP_DEP_DATE END), (CASE WHEN A.SEAT_CODE = 0 THEN A.ARR_DATE ELSE S.ARR_ARR_DATE END)) + 1 AS DATES
					, DBO.XN_COM_GET_TEAM_NAME(A.NEW_CODE) AS TEAM_NAME
					, DBO.XN_COM_GET_EMP_NAME(A.NEW_CODE) AS NEW_NAME
					, ISNULL(D.AGT_GRADE, '''') AS AGT_GRADE, D.MEM_CODE, D.GENDER, D.BIRTH_DATE							
				FROM LIST B LEFT JOIN PKG_DETAIL A WITH(NOLOCK)
					ON A.PRO_CODE = B.PRO_CODE  
						LEFT OUTER JOIN dbo.PRO_TRANS_SEAT S WITH(NOLOCK) ON A.SEAT_CODE = S.SEAT_CODE
						LEFT JOIN AGT_MEMBER D WITH(NOLOCK) ON A.TC_CODE = D.MEM_CODE 
				ORDER BY ' + @SORTING 
			...
			SET @SQLSTRING = N'
					...
					(
						...
						,STUFF((
								...
								LEFT OUTER JOIN dbo.PRO_TRANS_SEAT S WITH(NOLOCK, FORCESEEK) ON P.SEAT_CODE = S.SEAT_CODE
