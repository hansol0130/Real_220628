...
/*================================================================================================================
■ USP_NAME					: XP_TRAVEL_REPORT_ASG_LIST_SELECT
■ DESCRIPTION				: 사외업무시스템 인솔자관리 배정행사 리스트 검색
■ INPUT PARAMETER			: 
...
■ EXEC						: 
	...
	exec XP_TRAVEL_REPORT_ASG_LIST_SELECT @page_index, @page_size, @total_count output, @key, @order_by
	SELECT @TOTAL_COUNT
■ MEMO						: 
------------------------------------------------------------------------------------------------------------------
■ CHANGE HISTORY                   
...
CREATE  PROCEDURE [dbo].[XP_TRAVEL_REPORT_ASG_LIST_SELECT]
(
	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	SET @SQLSTRING = N'
				WITH LIST AS
				(
					SELECT A.PRO_CODE --행사코드
					FROM PKG_DETAIL A WITH(NOLOCK)
					LEFT OUTER JOIN dbo.TRAVEL_REPORT_MASTER V WITH(NOLOCK) ON A.PRO_CODE = V.PRO_CODE
