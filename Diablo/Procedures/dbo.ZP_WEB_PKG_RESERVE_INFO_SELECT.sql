...
GO
/*================================================================================================================
■ USP_NAME					: ZP_WEB_PKG_RESERVE_INFO_SELECT
■ DESCRIPTION				: 예약된 상품 정보 OR 상품정보 보기 
■ INPUT PARAMETER			:
■ OUTPUT PARAMETER			: 
■ EXEC						: 
	ZP_WEB_PKG_RESERVE_INFO_SELECT 'FPP153-190507',0,''
	ZP_WEB_PKG_RESERVE_INFO_SELECT '',0,'RP1902200997'
	ZP_WEB_PKG_RESERVE_INFO_SELECT '',0,'RP1311121772'
...
------------------------------------------------------------------------------------------------------------------
   ...
   2019-11-20		안병호			출도착 항공사명 추가 ARR_TRANS_NAME, DEP_TRANS_NAME, 입력일 추가 NEW_DATE
   2020-02-18	    임원묵			XP_WEB_PKG_RESERVE_INFO_SELECT -> ZP_WEB_PKG_RESERVE_INFO_SELECT 로 복사 생성   
================================================================================================================*/ 
CREATE  PROC [dbo].[ZP_WEB_PKG_RESERVE_INFO_SELECT]
	@PRO_CODE	PRO_CODE ,
	@PRICE_SEQ	INT ,
	@RES_CODE	RES_CODE 
AS 
BEGIN
	...
	BEGIN
		...
		SELECT 
			...
			(SELECT KOR_NAME FROM PUB_AIRLINE WITH(NOLOCK) WHERE AIRLINE_CODE = D.DEP_TRANS_CODE ) AS DEP_TRANS_NAME,
			(SELECT KOR_NAME FROM PUB_AIRLINE WITH(NOLOCK) WHERE AIRLINE_CODE = D.ARR_TRANS_CODE ) AS ARR_TRANS_NAME,
