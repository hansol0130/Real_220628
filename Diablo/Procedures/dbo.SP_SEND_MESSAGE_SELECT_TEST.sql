...
GO
/*================================================================================================================
■ USP_NAME					: SP_SEND_MESSAGE_SELECT_JUNGUN
■ DESCRIPTION				: 테스트중
■ INPUT PARAMETER			: 
■ OUTPUT PARAMETER			: 
■ EXEC						: 
	DECLARE @p5 INT
	SET @p5=NULL
	EXEC SP_SEND_MESSAGE_SELECT_TEST @PAGE_INDEX=1,@PAGE_SIZE=20,@KEY=N'CusNo=15',@ORDER_BY=0,@TOTAL_COUNT=@p5 OUTPUT
	SELECT @p5

	DECLARE @p5 INT
	SET @p5=NULL
	EXEC SP_SEND_MESSAGE_SELECT_TEST @PAGE_INDEX=1,@PAGE_SIZE=20,@KEY=N'Phone=01034441147&StartDate=2018-06-01&EndDate=2018-07-03',@ORDER_BY=0,@TOTAL_COUNT=@p5 OUTPUT
	SELECT @p5
■ MEMO						: 
------------------------------------------------------------------------------------------------------------------
■ CHANGE HISTORY                   
...
------------------------------------------------------------------------------------------------------------------
   2018-05-28		정지용			최초생성   
   2018-06-19		김성호			신규 SMS 발송내역 포함으로 수정
   2018-07-02		정지용			페이징 추가 ( 인덱스 걸어야함 넘 느림 )
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[SP_SEND_MESSAGE_SELECT_TEST]
(
 	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	DECLARE @CODE VARCHAR(20), @AGT_CODE VARCHAR(10), @CUS_NO INT, @START_DATE VARCHAR(10), @END_DATE VARCHAR(10), @PHONE_NUM VARCHAR(20);
	...
	FROM (
		...
		INNER JOIN (
			SELECT AA.SN, AA.PHONE_NUM, CONVERT(DATETIME, STUFF(STUFF(STUFF(ISNULL(AA.SMS_SND_DTM, AA.SND_DTM), 9, 0, '' ''), 12, 0, '':''), 15, 0, '':'')) AS SND_DATE
			FROM SEND.dbo.MZSENDTRAN AA WITH(NOLOCK) WHERE AA.PHONE_NUM IS NOT NULL
			UNION ALL
			SELECT BB.SN, BB.PHONE_NUM, CONVERT(DATETIME, STUFF(STUFF(STUFF(ISNULL(BB.SMS_SND_DTM, BB.SND_DTM), 9, 0, '' ''), 12, 0, '':''), 15, 0, '':'')) AS SND_DATE
		...
		FROM SEND.DBO.ALT_MESSAGE_MASTER A WITH(NOLOCK)
		INNER JOIN (
			SELECT AA.TRAN_PR, AA.TRAN_PHONE AS [PHONE_NUM], AA.TRAN_RSLTDATE AS SND_DATE FROM SEND.dbo.em_tran AA WITH(NOLOCK)
			UNION ALL
			SELECT BB.TRAN_PR, BB.TRAN_PHONE AS [PHONE_NUM], BB.TRAN_RSLTDATE AS SND_DATE FROM SEND.dbo.em_log BB WITH(NOLOCK)
	...
	(
		-- 알림톡 + SMS (엠엔와이즈)
		SELECT ''A'' AS [TYPE],
			A.SN AS [SND_NO], A.SND_TYPE, A.SND_NUMBER, A.BODY, A.SND_RESULT, A.SND_RESULT_MESSAGE, 
	...
	FROM TOTAL_LIST A
	LEFT JOIN DIABLO.DBO.EMP_MASTER_DAMO B WITH(NOLOCK) ON A.NEW_CODE = B.EMP_CODE';
