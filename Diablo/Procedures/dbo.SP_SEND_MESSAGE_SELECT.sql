...
GO
/*================================================================================================================
■ USP_NAME					: SP_SEND_MESSAGE_SELECT
■ DESCRIPTION				: 메시지 전송 조회
■ INPUT PARAMETER			: 
■ OUTPUT PARAMETER			: 
■ EXEC						: 

	EXEC SP_SEND_MESSAGE_SELECT 'RP1806041096', '', ''
	EXEC SP_SEND_MESSAGE_SELECT_TEST 'XXF777-180627', '', NULL
...
------------------------------------------------------------------------------------------------------------------
   ...
   2018-08-14		정지용			SORT 추가

================================================================================================================*/ 
CREATE PROCEDURE [dbo].[SP_SEND_MESSAGE_SELECT]
(
	@CODE		VARCHAR(20) = '',
	@AGT_CODE	VARCHAR(10) = '',
	@CUS_NO		INT = 0
...
BEGIN
	...
	DECLARE @SQLSTRING NVARCHAR(MAX), @PARMDEFINITION NVARCHAR(1000), @WHERE1 NVARCHAR(1000) = '', @WHERE2 NVARCHAR(1000) = '';
	...
	WITH ALRIM_LIST AS (
		...
		' + @WHERE1 + N' AND A.TRAN_SN IS NULL
		UNION ALL
		-- SMS(엠엔와이즈)
		SELECT
			A.*,
		...
		' + @WHERE1 + N' AND A.TRAN_SN IS NOT NULL
	)'
	...
	(
		...
		--LEFT JOIN DIABLO.DBO.EMP_MASTER_DAMO C WITH(NOLOCK) ON A.NEW_CODE = C.EMP_CODE
		UNION ALL
		-- SMS (신안정보통신)
		SELECT ''S'' AS [TYPE], 
			CONVERT(VARCHAR(100), A.SND_NO) AS SND_NO, A.SND_TYPE, A.SND_NUMBER, A.BODY, CONVERT(VARCHAR(1), A.SND_RESULT) AS [SND_RESULT], ''발송 성공'' AS [SND_RESULT_MESSAGE], 
			A.RCV_NUMBER1, A.RCV_NUMBER2, A.RCV_NUMBER3, A.RCV_NAME, 
			A.NEW_CODE, A.NEW_DATE, A.SND_DATE, A.SND_METHOD, A.RES_CODE, A.CUS_NO, NULL, NULL
		FROM RES_SND_SMS A WITH(NOLOCK) 
		' + @WHERE2 + N'
	)
	SELECT A.*, B.KOR_NAME AS [NEW_NAME]
	FROM TOTAL_LIST A
	LEFT JOIN DIABLO.DBO.EMP_MASTER_DAMO B WITH(NOLOCK) ON A.NEW_CODE = B.EMP_CODE ORDER BY A.SND_DATE ASC;';
