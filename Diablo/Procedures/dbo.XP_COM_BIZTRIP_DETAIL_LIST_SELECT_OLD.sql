...
declare @p5 int
set @p5=16
exec XP_COM_BIZTRIP_DETAIL_LIST_SELECT @PAGE_INDEX=1,@PAGE_SIZE=10,@KEY=N'agentCode=93258&EmpSeq=102&OrderBy=1',@ORDER_BY=1,@TOTAL_COUNT=@p5 output
select @p5
...
================================================================================================================*/ 
CREATE PROC [dbo].[XP_COM_BIZTRIP_DETAIL_LIST_SELECT_OLD]
	@PAGE_INDEX		INT,
	@PAGE_SIZE		INT,
...
BEGIN
	...
	DECLARE @WHERE NVARCHAR(4000), @SORT_STRING NVARCHAR(100), @RES_STATE_WHERE NVARCHAR(1000), @SORT_STRING2 NVARCHAR(50);
	DECLARE @AGT_CODE VARCHAR(10), @EMP_SEQ INT, @APPROVAL_CODE VARCHAR(10), @RES_STATE VARCHAR(1), @START_DATE VARCHAR(20), @END_DATE VARCHAR(20);
	--DECLARE @CANCEL_RESERVE_YN VARCHAR(1), @WHERE_RES_CANCEL NVARCHAR(100) = ''
	DECLARE @CANCEL_RESERVE_YN VARCHAR(1), @BT_STATE_WHERE NVARCHAR(200) = ''
	DECLARE @SEARCH_BT_STATE INT 
...
--		@STATE_CODE = DBO.FN_PARAM(@KEY, 'StateCode'),
	...
	--SELECT @WHERE = 'WHERE A.AGT_CODE = @AGT_CODE AND (A.NEW_SEQ = @EMP_SEQ OR E.EMP_SEQ = @EMP_SEQ)'
	SELECT @WHERE = 'WHERE A.AGT_CODE = @AGT_CODE AND (
	...
	--IF LEN(@STATE_CODE) > 0 OR @STATE_CODE = ','
	--BEGIN
	--	SET @WHERE = @WHERE + ' AND A.BT_STATE IN (SELECT DATA FROM DBO.FN_SPLIT(@STATE_CODE, '','') WHERE Data <> '''')'
	--END
	...
	SELECT @SORT_STRING = (  
		CASE @ORDER_BY
			...
			WHEN 2 THEN ' Z.BT_STATE, (CASE WHEN A.BT_START_DATE <= GETDATE() THEN ''9999'' ELSE A.BT_START_DATE END) '
			WHEN 3 THEN ' Z.BT_STATE, (CASE WHEN A.BT_TIME_LIMIT <= GETDATE() THEN ''9999'' ELSE A.BT_TIME_LIMIT END) '
			WHEN 4 THEN ' Z.BT_STATE, (CASE WHEN A.BT_TIME_LIMIT <= GETDATE() THEN ''9999'' ELSE A.BT_TIME_LIMIT END) '
    ...
    SET @SQLSTRING = @SQLSTRING + CONVERT(NVARCHAR(MAX), N'
		...
		FROM (
			...
			' + @WHERE + N'
			GROUP BY A.AGT_CODE, A.BT_CODE, A.BT_END_DATE 
		) Z
		' + @BT_STATE_WHERE + N';
		')
	...
	SELECT @SQLSTRING = @SQLSTRING + CONVERT(NVARCHAR(MAX), N'
		...
		WITH RES_LIST AS
		(
			SELECT A.AGT_CODE, A.BT_CODE, Z.LAST_NEW_DATE, Z.BT_STATE, (
					SELECT COUNT(*) 
					FROM COM_BIZTRIP_DETAIL AA WITH(NOLOCK) 
					INNER JOIN RES_MASTER_DAMO BB WITH(NOLOCK) ON AA.RES_CODE = BB.RES_CODE ' + REPLACE(@BT_STATE_WHERE, 'WHERE', 'AND') + N'
					WHERE AA.AGT_CODE = Z.AGT_CODE AND AA.BT_CODE = Z.BT_CODE
				) AS [RES_COUNT] , Z.BT_END_DATE 
			FROM (
				SELECT A.AGT_CODE, A.BT_CODE, MAX(C.NEW_DATE) AS [LAST_NEW_DATE]--, COUNT(*) AS [RES_COUNT]
					, (CASE
						WHEN MIN(C.RES_STATE) < 7 THEN 0
						WHEN MIN(C.RES_STATE) = 7 THEN 7
						ELSE 9
		...
		(
			SELECT A.AGT_CODE, A.BT_CODE, B.RES_CODE, COUNT(*) AS [CUS_COUNT]
			FROM RES_LIST A
			INNER JOIN COM_BIZTRIP_DETAIL B WITH(NOLOCK) ON A.AGT_CODE = B.AGT_CODE AND A.BT_CODE = B.BT_CODE
			INNER JOIN RES_MASTER_DAMO C WITH(NOLOCK) ON B.RES_CODE = C.RES_CODE
			INNER JOIN RES_CUSTOMER_DAMO D WITH(NOLOCK) ON C.RES_CODE = D.RES_CODE
			WHERE C.RES_STATE <= 7 AND D.RES_STATE = 0 
		...
		INNER JOIN RES_MASTER_DAMO C WITH(NOLOCK) ON B.RES_CODE = C.RES_CODE ') + @RES_STATE_WHERE + CONVERT(NVARCHAR(MAX), N'
		LEFT JOIN CUS_LIST D ON Z.AGT_CODE = D.AGT_CODE AND Z.BT_CODE = D.BT_CODE AND C.RES_CODE = D.RES_CODE
