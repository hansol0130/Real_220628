...
/*================================================================================================================
■ USP_NAME					: XP_ASG_EVT_REPORT_LIST_SELECT
...
■ EXEC						: 
	...
	SELECT @PAGE_INDEX=1,@PAGE_SIZE=10,@KEY=N'EmpCode=T130001&ProCode=APP0504-130327TG5&ProName=&SDate=2013-04-01&EDate=2013-04-30&ReportState=&signCode=',@ORDER_BY=4

	exec XP_ASG_EVT_REPORT_LIST_SELECT @page_index, @page_size, @total_count output, @key, @order_by
	SELECT @TOTAL_COUNT
■ MEMO						: 
------------------------------------------------------------------------------------------------------------------
■ CHANGE HISTORY                   
...
------------------------------------------------------------------------------------------------------------------
   2013-03-29		오인규			최초생성    출장보고서 상태값 검색 추가해야함
   2019-01-08		이명훈			출장보고서 리스트 페이징 제거
   2019-04-11		이명훈			출장보고서 테이브 변경
================================================================================================================*/ 

CREATE  PROCEDURE [dbo].[XP_ASG_EVT_REPORT_LIST_SELECT]
(
	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	BEGIN
		...
		BEGIN
		     IF @REPORT_STATE = '0'
				SET @WHERE = @WHERE + ' AND (V.OTR_STATE IS NULL OR V.OTR_STATE = 0 OR V.OTR_STATE = 1)'
			 ELSE IF @REPORT_STATE = '3'
				SET @WHERE = @WHERE + ' AND V.OTR_STATE IN (2, 3)'
			 ELSE
	...
	SET @PARMDEFINITION = N'@PAGE_INDEX  INT, @PAGE_SIZE  INT, @TOTAL_COUNT INT OUTPUT, @EMP_CODE CHAR(7), @PRO_CODE VARCHAR(20),@PRO_NAME NVARCHAR(100), @START_DATE DATETIME ,@END_DATE	DATETIME ,@REPORT_STATE INT ,@SIGN_CODE CHAR(1)';

	PRINT @SQLSTRING
		
	EXEC SP_EXECUTESQL @SQLSTRING, @PARMDEFINITION, @PAGE_INDEX, @PAGE_SIZE, @TOTAL_COUNT OUTPUT, @EMP_CODE, @PRO_CODE,@PRO_NAME , @START_DATE ,@END_DATE,@REPORT_STATE,@SIGN_CODE;
