...
/*================================================================================================================
■ USP_NAME					: ZP_WEB_PKG_DETAIL_SELECT
...
■ EXEC						: 
	...
	exec ZP_WEB_PKG_DETAIL_SELECT @PRO_CODE output, @PRICE_SEQ OUTPUT  ,@SHOW_ALL_YN = '' 
	...
	exec ZP_WEB_PKG_DETAIL_SELECT @PRO_CODE output, @PRICE_SEQ OUTPUT
...
------------------------------------------------------------------------------------------------------------------
   ...
   2020-02-18	    임원묵			XP_WEB_PKG_DETAIL_SELECT -> ZP_WEB_PKG_DETAIL_SELECT 로 복사 생성
   2020-02-19	    임검제			SP_WEB_PKG_DETAIL_SCHEDULE_SELECT => ZP_WEB_PKG_DETAIL_SCHEDULE_SELECT 내부호출 SP이름변경
   2020-03-06		임검제			PKG_MASTER 테이블 PKG_TOOLTIP 컬럼 조회 추가
   2020-06-22		김영민			CLEAN_YN 컬럼추가
   2020-07-13		김영민			SINGLE_YN 컬럼추가(단일상품)
   2020-08-19		김영민			국내무박상품으로인한 (DEP_DEP_DATE,DEP_ARR_DATE,ARR_DEP_DATE,ARR_ARR_DATE)수정
   2020-12-29		홍종우			상품판매 판매기한 추가 ( PKG_DETAIL_EXTENSION 테이블 DEADLINE, USE_YN 컬럼 추가)
   2021-01-07       오준혁           PKG_MASTER의 SHOW_YN 이 'N'인 경우 예약마감으로 세팅
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[ZP_WEB_PKG_DETAIL_SELECT]
(
...
BEGIN
	...
	IF @PRICE_SEQ = 0
	BEGIN
	    SELECT TOP 1 @PRICE_SEQ = PRICE_SEQ
	...
	SELECT PC.*, A.PRO_CODE, A.PRO_NAME, A.MASTER_CODE, A.TRANSFER_TYPE, A.SEAT_CODE, A.TOUR_NIGHT, A.TOUR_DAY, A.FAKE_COUNT, A.MAX_COUNT, A.MIN_COUNT
		, A.TOUR_JOURNEY, A.DEP_CFM_YN, A.CONFIRM_YN, A.DEP_DATE, A.ARR_DATE, A.SALE_TYPE, A.FIRST_MEET, A.MEET_COUNTER, A.PKG_TOUR_REMARK, A.PKG_SUMMARY, A.HOTEL_REMARK
	...
	LEFT JOIN PKG_DETAIL_EXTENSION G WITH(NOLOCK) ON A.PRO_CODE = G.PRO_CODE
	CROSS JOIN (
	...
	*/
	...
	--일정정보
	EXEC DBO.ZP_WEB_PKG_DETAIL_SCHEDULE_SELECT @PRO_CODE, @PRICE_SEQ
	-- 공동경비정보
