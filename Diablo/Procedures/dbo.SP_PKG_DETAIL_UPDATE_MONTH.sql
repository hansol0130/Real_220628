...
GO
/*================================================================================================================    
■ USP_Name			: SP_PKG_DETAIL_UPDATE_MONTH      
■ Description		: 행사 월별 수정    
■ Input Parameter   :                      
  @PRO_CODE			: 상품코드    
  @DAYS				: 일자들    
  @UPDATE_TYPE		: 복사타입
...
■ Output Parameter	:                      
■ Output Value		:                     
■ Exec				: EXEC SP_PKG_DETAIL_UPDATE_MONTH @PRO_CODE=N'CPP006-090401',@DAYS=N'2009-07-05|2009-08-10|2009-08-12|2009-08-19|2009-08-28|',@UPDATE_TYPE=N'10000',@EMP_CODE=N'2008011'    
■ Author			: 김성호    
■ Date				: 2009-07-22    
■ Memo				:    
------------------------------------------------------------------------------------------------------------------    
...
================================================================================================================*/     
CREATE PROCEDURE [dbo].[SP_PKG_DETAIL_UPDATE_MONTH]
(    
	@PRO_CODE			VARCHAR(20),    
	@DAYS				VARCHAR(1000),    
...
BEGIN
	...
	DECLARE @NEW_MASTER_CODE VARCHAR(10), @NEW_PRO_CODE VARCHAR(20), @BIT_CODE VARCHAR(10)    
...
BEGIN 
	...
	BEGIN    
		...
		BEGIN    
			...
			IF @ALL_PRODUCT_YN = 'Y'
			BEGIN
				SET @NEW_PRO_CODE = (@NEW_MASTER_CODE + '-' + SUBSTRING(ISNULL(REPLACE(@DAY, '-', ''), ''), 3, 10) + '%')
			END
			ELSE
			BEGIN
				SET @NEW_PRO_CODE = (@NEW_MASTER_CODE + '-' + SUBSTRING(ISNULL(REPLACE(@DAY, '-', ''), ''), 3, 10) + ISNULL(@BIT_CODE, ''))
			END
						...
						BEGIN
							...
							BEGIN    
								...
								SELECT @NEW_SEAT_CODE = (
									...
									WHERE BAS.SEAT_CODE = @SEAT_CODE AND PTS.DEP_DEP_DATE = @DAY
										AND PTS.ARR_DEP_DATE = DATEADD(DAY, @DIFF_DAY, @DAY)    
										AND PTS.DEP_ARR_DATE = DATEADD(DAY, @DEP_DIFF, @DAY)    
										AND PTS.ARR_ARR_DATE = DATEADD(DAY, @ARR_DIFF, @DAY)    
									ORDER BY SEAT_CODE

									--SELECT TOP 1 PTS.SEAT_CODE
									--FROM PRO_TRANS_SEAT AS PTS WITH(NOLOCK)     
									--INNER JOIN (
									--	SELECT TOP 1 * FROM PRO_TRANS_SEAT WITH(NOLOCK)     
									--	WHERE SEAT_CODE = @SEAT_CODE
									--) AS BAS    
									--	ON PTS.DEP_TRANS_CODE =  BAS.DEP_TRANS_CODE AND PTS.DEP_TRANS_NUMBER = BAS.DEP_TRANS_NUMBER    
									--	AND PTS.DEP_DEP_AIRPORT_CODE = BAS.DEP_DEP_AIRPORT_CODE AND PTS.DEP_ARR_AIRPORT_CODE = BAS.DEP_ARR_AIRPORT_CODE    
								...
								BEGIN    
									...
									SELECT @DAY, 2, '좌석 패턴 없음'
								END    
							END     
							ELSE    
							BEGIN
								INSERT INTO #MSG_TEMP     
								SELECT @DAY, 2, '교통편 항공이 아님'
							END     
						END
...
--							print @query
						...
						BEGIN
							INSERT INTO #MSG_TEMP
							SELECT @DAY, 3, '예약 존재 가격/호텔 제외 수정'
						END
				...
				BEGIN    
					-- LOG INSERT    
					INSERT INTO #MSG_TEMP     
					SELECT @DAY, 2, (@NEW_PRO_CODE + ' 기준행사')    
				END    
			END    
			ELSE    
			BEGIN    
