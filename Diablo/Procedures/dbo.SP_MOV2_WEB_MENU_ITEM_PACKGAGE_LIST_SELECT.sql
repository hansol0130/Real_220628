...
GO
/*================================================================================================================
■ USP_NAME					: SP_MOV2_WEB_MENU_ITEM_PACKGAGE_LIST_SELECT (XP_WEB_MENU_ITEM_PACKGAGE_LIST_SELECT)
...
■ EXEC						: 
	...
	exec SP_MOV2_WEB_MENU_ITEM_PACKGAGE_LIST_SELECT 'VGT', '10206', '2', 5, 'D'
...
------------------------------------------------------------------------------------------------------------------
   ...
   2017-08-21		IBSOLUTION		상품에 아이콘 표시 (VR/EVENT)
================================================================================================================*/ 

CREATE  PROCEDURE [dbo].[SP_MOV2_WEB_MENU_ITEM_PACKGAGE_LIST_SELECT]
(
	@SITE_CODE	VARCHAR(3),
...
BEGIN
	...
	(
		SELECT ''M'' AS [FLAG], A.SITE_CODE, A.MENU_CODE, A.SEC_CODE, A.GRP_SEQ, A.ITEM_SEQ, A.MASTER_CODE
			...
			, (SELECT COUNT(*) FROM VR_CONTENT V2 WITH(NOLOCK) WHERE B.MASTER_CODE = V2.MASTER_CODE) AS [VR_COUNT]
			...
			, B.TAG AS [TAG]
			, (SELECT COUNT(*) FROM RES_MASTER_DAMO C2 WITH (NOLOCK) WHERE C2.NEW_DATE > DATEADD(DY, -7, GETDATE()) AND C2.MASTER_CODE = B.MASTER_CODE) AS [RESERVE_COUNT]
	...
	SELECT ' + (CASE @TOP_COUNT WHEN 0 THEN '' ELSE 'TOP (@TOP_COUNT) ' END) + N'
		...
		, B.GRP_TITLE, B.GRP_CODE, B.GRP_DESC, A.ORDER_NO
		, C.DEP_DATE, C.TOUR_NIGHT, C.TOUR_DAY, C.TOUR_JOURNEY, D.SIGN_CODE, D.ATT_CODE
	...
	LEFT JOIN PKG_DETAIL C WITH(NOLOCK) ON C.PRO_CODE = Z.PRO_CODE
	LEFT JOIN PKG_MASTER D WITH(NOLOCK) ON C.MASTER_CODE = D.MASTER_CODE
	...
	EXEC SP_EXECUTESQL @SQLSTRING, @PARMDEFINITION, @SITE_CODE, @MENU_CODE, @SEC_CODE, @TOP_COUNT;
