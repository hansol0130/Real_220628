...
-- =============================================
-- Author:		이규식
-- Create date: 2010-07-06
-- Description:	예약자별 수익현황
-- History
-- 2010-8-4 엔코어 튜닝으로 인해 이전 SP 백업
-- =============================================
CREATE PROCEDURE [dbo].[SP_SET_GET_RES_COMPLETE_LIST_4_BACKUP]
(
	@DATE_TYPE	INT,
	@EMP_TYPE	INT,
	@START_DATE	DATETIME,
...
BEGIN
	...
	SET @SQLSTRING =
		N'SELECT
			...
			(SELECT KOR_NAME FROM PUB_REGION Z  WITH(NOLOCK) WHERE Z.REGION_CODE = E.SIGN_CODE) AS REGION_NAME
		FROM SET_MASTER A WITH(NOLOCK)
		INNER JOIN PKG_DETAIL B  WITH(NOLOCK) ON B.PRO_CODE = A.PRO_CODE
		INNER JOIN RES_MASTER_DAMO C WITH(NOLOCK) ON C.PRO_CODE = B.PRO_CODE
		INNER JOIN EMP_MASTER D WITH(NOLOCK) ON D.EMP_CODE = B.NEW_CODE
		INNER JOIN PKG_MASTER E WITH(NOLOCK) ON E.MASTER_CODE = B.MASTER_CODE
		WHERE 1 = 1'
	...
	BEGIN
		...
		SET @SQLSTRING = @SQLSTRING + ' AND EXISTS (SELECT TOP 1 1 FROM PKG_DETAIL_SCH_CITY Z  WITH(NOLOCK) WHERE Z.PRO_CODE = A.PRO_CODE AND Z.CITY_CODE IN ( SELECT CITY_CODE FROM PUB_CITY  WITH(NOLOCK) WHERE NATION_CODE IN (' + @NATION_CODE + ')))'
	END
	...
	BEGIN
		...
		SET @SQLSTRING = @SQLSTRING + ' AND EXISTS (SELECT TOP 1 1 FROM PKG_DETAIL_SCH_CITY Z  WITH(NOLOCK) WHERE Z.PRO_CODE = A.PRO_CODE AND CITY_CODE IN (' + @CITY_CODE + '))' 
	END
	...
	BEGIN
		...
		B.PRO_NAME,
		B.DEP_DATE,
		(SELECT KOR_NAME FROM PUB_REGION Z  WITH(NOLOCK) WHERE Z.SIGN = C.SIGN_CODE) AS REGION_NAME,
