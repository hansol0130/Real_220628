...
■ Server     :         
■ Database     : DIABLO        
■ USP_Name     : ZP_ETBS_EVT_SEARCH_LIST_SELECT        
■ Description    : 기획전/이벤트 리스트 검색 (이제너두) 국내제외       
■ Input Parameter   :         
...
■ Exec      :         
 ...
 declare @total int        
 exec [dbo].[ZP_ETBS_EVT_SEARCH_LIST_SELECT] 1, 20, @total output, 'PlanType=A&IsEvent=N&AttrCode=T&EventName=', 0        
        
declare @p5 int        
set @p5=0        
exec ZP_ETBS_EVT_SEARCH_LIST_SELECT @PAGE_INDEX=1,@PAGE_SIZE=12,@KEY=N'PlanType=M&IsEvent=Y&IsClose=',@ORDER_BY=0,@TOTAL_COUNT=@p5 output        
select @p5        
...
---------------------------------------------------------------------------------------------------        
 2013-05-27  김영민   SIGN_CODE <> D 국내제외        
-------------------------------------------------------------------------------------------------*/         
CREATE PROCEDURE [dbo].[ZP_ETBS_EVT_SEARCH_LIST_SELECT] 
...
(        
 ...
 WHERE 1=1  AND A.SIGN_CODE <> ''D'' ' + @WHERE + '        
