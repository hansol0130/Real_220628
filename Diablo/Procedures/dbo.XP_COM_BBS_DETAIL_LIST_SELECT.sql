...
■ USP_NAME					: XP_COM_BBS_DETAIL_LIST_SELECT
...
■ EXEC						: 
	...
	exec XP_COM_BBS_DETAIL_LIST_SELECT 1, 10, 10, N'AgtCode=93086&MasterSeq=1&SearchType=&SearchText=&IsNotice=Y&TitleResize=0&LayoutSeq=1', 1
...
------------------------------------------------------------------------------------------------------------------
   2016-01-20		저스트고-백경훈  최초생성
   2016-04-07		저스트고-이유라	ERP에도 쓰일수있도록 수정
   2016-07-26		저스트고-이유라	SEARCH_TYPE 6번 국가 검색 추가
   2016-07-26		박형만			COMMENT_DATE  ,  최종 답변작성(수정)일 구하기 
   2016-12-26		저스트고-이유라  국가없을시 '전체'로 표기
   2017-03-27		김성호			회원사 공지를 최우선으로 정렬하도록 수정
================================================================================================================*/ 
CREATE  PROCEDURE [dbo].[XP_COM_BBS_DETAIL_LIST_SELECT]
(
	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	DECLARE @MASTER_SEQ INT,@AGT_CODE VARCHAR(20), @BOARD_TYPE VARCHAR(1), @CATEGORY_SEQ INT, @NATION_CODE VARCHAR(30), @REGION_CODE VARCHAR(30), @BT_CODE varchar(20)
		, @SEARCH_TYPE VARCHAR(1), @SEARCH_TEXT VARCHAR(100), @NOTICE_YN VARCHAR(1), @DEL_YN VARCHAR(1), @EMPLOYEE_YN VARCHAR(1), @NTYPE VARCHAR(1);
	DECLARE	@ERP_YN VARCHAR(1), @START_DATE VARCHAR(20), @END_DATE VARCHAR(20), @REQUEST_SEQ INT, @COMMENT_COUNT VARCHAR(1), @MANAGER_EMP_CODE VARCHAR(7), @EMP_TYPE VARCHAR(50), @EMP_SEQ VARCHAR(5);
	...
	BEGIN
		SELECT
			...
			@ERP_NOTICE_UNION = ''
	END
	ELSE IF (@MASTER_SEQ = '3')
	BEGIN 
		SELECT
			@NOTICE_WHERE = 'WHERE 1 <> 1',
			@ERP_NOTICE_UNION = 'SELECT * FROM LIST  UNION ALL'
	END
	ELSE
	BEGIN
	...
	IF(@MASTER_SEQ = '4' AND @EMP_SEQ <> '')
	BEGIN
		SELECT
			@WHERE = @WHERE + ' AND A.NEW_SEQ = @EMP_SEQ ',
			@NOTICE_WHERE = @NOTICE_WHERE + ' AND A.NEW_SEQ = @EMP_SEQ '
	END
	...
	END
	ELSE IF (@AGT_CODE <>'' AND @MASTER_SEQ = '3')
	BEGIN
	...
	BEGIN
		...
		IF @SEARCH_TYPE <> 4 AND @SEARCH_TYPE <> 5 AND @SEARCH_TYPE <> 6
		...
		SET @WHERE = @WHERE + (
			CASE @SEARCH_TYPE
				...
				WHEN '4' THEN ' AND (A.NEW_SEQ IN (SELECT EMP_SEQ FROM COM_EMPLOYEE AA WHERE AA.KOR_NAME LIKE ''%'' + @SEARCH_TEXT + ''%'')) '
				WHEN '5' THEN ' AND A.BT_CODE = @SEARCH_TEXT '
				WHEN '6' THEN ' AND (A.NATION_CODE IN (SELECT NATION_CODE FROM PUB_NATION WHERE KOR_NAME LIKE ''%'' + @SEARCH_TEXT + ''%'')) '
	...
	' + @WHERE + N';
	SELECT @TOTAL_COUNTS =COUNT(*) 
	FROM COM_BBS_DETAIL A
	WHERE  A.MASTER_SEQ = @MASTER_SEQ AND A.NOTICE_YN = ''Y'' AND A.DEL_YN = ''N'' AND (A.AGT_CODE = @AGT_CODE OR A.ALL_NOTICE_YN = ''Y''
	OR STUFF(( SELECT '','' + B.AGT_CODE FROM COM_BBS_NOTICE B WHERE A.MASTER_SEQ = B.MASTER_SEQ AND A.BOARD_SEQ = B.BOARD_SEQ ORDER BY B.AGT_NAME FOR XML PATH ('''')),1,1,'''')  LIKE  ''%'' + @AGT_CODE + ''%'') ;

	WITH LIST AS
	(
		SELECT A.NOTICE_YN, A.MASTER_SEQ, A.BOARD_SEQ, 3 AS [FLAG]
		FROM COM_BBS_DETAIL A WITH(NOLOCK)
		LEFT JOIN COM_MANAGER B ON A.AGT_CODE = B.AGT_CODE AND A.CATEGORY_SEQ = B.MANAGER_TYPE
		' + @WHERE + N'
	...
	SELECT
		Z.FLAG,
		(case when A.NOTICE_YN = ''Y'' then 0  else ((@TOTAL_COUNT + @TOTAL_COUNTS) -((@PAGE_INDEX - 1) * @PAGE_SIZE) - (ROW_NUMBER() OVER(ORDER BY A.master_seq desc)-1)) end) AS ROW_NUM,
		...
		(SELECT ISNULL(MAX(EDT_DATE),MAX(NEW_DATE)) FROM COM_BBS_DETAIL WITH(NOLOCK) WHERE MASTER_SEQ = A.MASTER_SEQ AND PARENT_SEQ = A.BOARD_SEQ AND BOARD_SEQ <> PARENT_SEQ )AS COMMENT_DATE ,
	...
	FROM (
		' + @ERP_NOTICE_UNION + '
		SELECT A.NOTICE_YN, A.MASTER_SEQ, A.BOARD_SEQ, (CASE WHEN A.EMP_CODE IS NULL THEN 1 ELSE 2 END) AS [FLAG]
		FROM COM_BBS_DETAIL A WITH(NOLOCK)
		' + @NOTICE_WHERE + ' 
	) Z
