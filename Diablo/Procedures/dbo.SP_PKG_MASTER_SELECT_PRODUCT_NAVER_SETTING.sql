...
/*================================================================================================================
■ USP_NAME					: SP_PKG_MASTER_SELECT_PRODUCT_NAVER_SETTING
■ DESCRIPTION				: 2019 네이버 패키지 상품연동
■ INPUT PARAMETER			: 
■ OUTPUT PARAMETER			: 
■ EXEC						: SP_PKG_MASTER_SELECT_PRODUCT_NAVER_SETTING ''
...
2019-10-14			박형만  3일치 보관 하도록 수정 
================================================================================================================*/ 
CREATE PROC [dbo].[SP_PKG_MASTER_SELECT_PRODUCT_NAVER_SETTING]
...
BEGIN
	...
	--SELECT CONVERT(DATETIME,@TMP_TARGET_DATE)
		...
		BEGIN
			...
			-- 3일전 테이블 삭제 
			SET @TARGET_DEL_TABLE =  @SOURCE_TABLE+'_'+ CONVERT(VARCHAR(8),DATEADD(DD,-3,CONVERT(DATETIME,@TMP_TARGET_DATE)),112 ) 
...
--SET @MASTER_CODE= 'EPP545'
	...
	SELECT MASTER_CODE , ISNULL(USER_RANK,999) FROM TMP_NAVER_MASTER WHERE USE_YN ='Y'   -- 2019-05-16 기준 80% 입력율 넘은것들 은 Y 로 처리 
...
--SELECT * FROM PKG_MASTER_AFFILIATE
	...
	BEGIN
		...
		BEGIN
			-- 전체일때 전체 삭제 
			TRUNCATE TABLE NAVER_PKG_MASTER 
		END 
		ELSE 
		BEGIN
			-- 하나일때, 하나만 삭제  
			DELETE NAVER_PKG_MASTER 
			WHERE MSTCODE IN (SELECT MASTER_CODE FROM #TMP_PRODUCT ) 
		END 

		-- 다시 등록 
		INSERT INTO  NAVER_PKG_MASTER
