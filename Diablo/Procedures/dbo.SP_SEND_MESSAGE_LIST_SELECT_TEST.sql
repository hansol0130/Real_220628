...
GO
/*================================================================================================================
■ USP_NAME					: SP_SEND_MESSAGE_LIST_SELECT_TEST
...
■ EXEC						: 
	...
	SET @p5=NULL
	EXEC SP_SEND_MESSAGE_LIST_SELECT_TEST @PAGE_INDEX=1,@PAGE_SIZE=15,@KEY=N'SndTypeCD=Y',@ORDER_BY=0,@TOTAL_COUNT=@p5 OUTPUT,@ALIM_SUCC_COUNT = @p6 OUTPUT, @ALIM_FAIL_COUNT = @p7 OUTPUT, @SMS_SUCC_COUNT = @p8 OUTPUT, @SMS_FAIL_COUNT = @p9 OUTPUT
	SELECT @p5, @p6, @p7, @p8, @p9

	DECLARE @p5 INT, @p6 INT, @p7 INT, @p8 INT, @p9 INT
	SET @p5=NULL
	EXEC SP_SEND_MESSAGE_LIST_SELECT_TEST @PAGE_INDEX=1,@PAGE_SIZE=10,@KEY=N'Phone=01032536841&StartDate=2018-06-01&EndDate=2018-07-03',@ORDER_BY=0,@TOTAL_COUNT=@p5 OUTPUT,@ALIM_SUCC_COUNT = @p6 OUTPUT, @ALIM_FAIL_COUNT = @p7 OUTPUT, @SMS_SUCC_COUNT = @p8 OUTPUT, @SMS_FAIL_COUNT = @p9 OUTPUT
	SELECT @p5, @p6, @p7, @p8, @p9
■ MEMO						: 
------------------------------------------------------------------------------------------------------------------
■ CHANGE HISTORY                   
...
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[SP_SEND_MESSAGE_LIST_SELECT_TEST]
(
 	@PAGE_INDEX  INT,
	@PAGE_SIZE  INT,
...
BEGIN
	...
	FROM (
		...
		INNER JOIN (
			...
			UNION ALL
			SELECT BB.SN, BB.PHONE_NUM, BB.SMS_SND_NUM, CONVERT(DATETIME, STUFF(STUFF(STUFF(ISNULL(BB.SMS_SND_DTM, BB.SND_DTM), 9, 0, '' ''), 12, 0, '':''), 15, 0, '':'')) AS SND_DATE, BB.REQ_DEPT_CD, BB.REQ_USR_ID, CASE WHEN ISNULL(BB.RSLT_CD, '''') = ''0000'' OR ISNULL(BB.SMS_RSLT_CD, '''') = ''0'' THEN ''S'' ELSE ''F'' END AS T_RESULT, (CASE WHEN BB.SMS_SND_DTM IS NULL THEN 117 ELSE 118 END) AS [SND_TYPE]
	...
	(
		...
		FROM (
			...
			INNER JOIN (
				...
				FROM SEND.dbo.MZSENDTRAN AA WITH(NOLOCK) WHERE AA.PHONE_NUM IS NOT NULL
				UNION ALL
				SELECT BB.SN, BB.PHONE_NUM, BB.SMS_SND_NUM, CONVERT(DATETIME, STUFF(STUFF(STUFF(ISNULL(BB.SMS_SND_DTM, BB.SND_DTM), 9, 0, '' ''), 12, 0, '':''), 15, 0, '':'')) AS SND_DATE, BB.REQ_DEPT_CD, BB.REQ_USR_ID
	...
	, ALRIM_LIST AS (
		...
		SELECT
			...
			B.PHONE_NUM as rcv_test_number,
		...
		SELECT
			A.*,
			118 AS [SND_TYPE],
			B.TRAN_CALLBACK as test_number,
			...
			B.TRAN_PHONE as rcv_test_number,
	...
	(
		-- 알림톡 + SMS (엠엔와이즈)
		SELECT ''A'' AS [TYPE], test_number,rcv_test_number,
