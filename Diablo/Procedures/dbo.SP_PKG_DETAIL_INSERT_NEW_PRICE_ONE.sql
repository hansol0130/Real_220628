...
GO
/*================================================================================================================    
■ USP_Name			: SP_PKG_DETAIL_INSERT_NEW_PRICE_ONE
■ Description		: 마스터 가격정보 하나를 행사로 복사
■ Input Parameter   : 
■ Output Parameter	:                      
■ Output Value		:                     
■ Exec				: 
...
================================================================================================================*/
CREATE PROCEDURE [dbo].[SP_PKG_DETAIL_INSERT_NEW_PRICE_ONE]
...
BEGIN
	...
	SELECT @NEW_PRICE_SEQ = (ISNULL(MAX(PRICE_SEQ), 0) + 1) FROM PKG_DETAIL_PRICE WITH(NOLOCK) WHERE PRO_CODE = @PRO_CODE
	...
	SELECT
		@PRO_CODE, @NEW_PRICE_SEQ, A.PRICE_NAME, A.SEASON, @SCH_SEQ, A.PKG_INCLUDE, A.PKG_NOT_INCLUDE
	...
	SELECT
		@PRO_CODE, @NEW_PRICE_SEQ, A.DAY_NUMBER, A.HTL_MASTER_CODE, A.SUP_CODE, A.STAY_TYPE, A.STAY_INFO, A.DINNER_1, A.DINNER_2, A.DINNER_3,A.DINNER_CODE_1,A.DINNER_CODE_2,A.DINNER_CODE_3
	...
	SELECT @PRO_CODE, @NEW_PRICE_SEQ, A.COST_SEQ, A.COST_NAME, A.CURRENCY, A.ADT_COST, A.CHD_COST, A.INF_COST, A.USE_YN
	...
	SELECT @PRO_CODE, @NEW_PRICE_SEQ, A.INOUT_CODE, A.IN_YN
	FROM PKG_MASTER_PRICE_INOUT A WITH(NOLOCK)
	WHERE A.MASTER_CODE = @MASTER_CODE AND A.PRICE_SEQ = @PRICE_SEQ 

	-- 마스터 상세와 관광일수 차이가 있을때 실행 - 남는 일자 삭제 및 모자른 일자 생성
	...
	FROM PKG_DETAIL_PRICE_HOTEL  WITH(NOLOCK)   WHERE PRO_CODE = @PRO_CODE AND PRICE_SEQ = @NEW_PRICE_SEQ
