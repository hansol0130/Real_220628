...
■ EXEC						: 
	...
	exec SP_STS_AIR_SALE_MONTH_LIST_TEST 0, 0, 1, 1, '2018-09-01', '2018-09-30', 'oz', '', '', ''
...
------------------------------------------------------------------------------------------------------------------
   ...
   2018-09-18		김성호			기간검색 가능하게 수정 (기간검색은 같은 월 이어야 하고 해당 월 기준으로 구간을 지정한다) 임시 차단
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[SP_STS_AIR_SALE_MONTH_LIST_TEST]
	@SELECT_TYPE	INT,
	@COMPANY		INT,
	@PRICE_TYPE		INT,
...
BEGIN  
	...
	--SELECT @STAND_DATE = CONVERT(DATETIME, (CONVERT(VARCHAR(8), GETDATE(), 121) + '01'))
	...
	-- GDS 구분
	IF @GDS_TYPE = 'BSP'
		SET @WHERE_COMPANY = @WHERE_COMPANY + ' AND A.GDS <> 10'
	ELSE IF @GDS_TYPE = 'ATR'
		SET @WHERE_COMPANY = @WHERE_COMPANY + ' AND A.GDS = 10'
	...
	(
		...
		FROM (
			...
			--WHERE (A.START_DATE >= @START_DATE AND A.START_DATE < DATEADD(DD, 1, @END_DATE) OR (A.END_DATE >= @START_DATE AND A.END_DATE < DATEADD(DD, 1, @END_DATE)))
		) A
