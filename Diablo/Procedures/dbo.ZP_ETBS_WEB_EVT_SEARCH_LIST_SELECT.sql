...
■ Server     :         
■ Database     : DIABLO        
■ USP_Name     : ZP_ETBS_WEB_EVT_SEARCH_LIST_SELECT        
...
■ Exec      :         
 ...
 declare @total int        
 exec [dbo].[ZP_ETBS_WEB_EVT_SEARCH_LIST_SELECT] 1, 20, @total output, 'PlanType=A&IsEvent=N&AttrCode=T&EventName=', 0        
        
declare @p5 int        
set @p5=0        
exec ZP_ETBS_WEB_EVT_SEARCH_LIST_SELECT @PAGE_INDEX=1,@PAGE_SIZE=12,@KEY=N'PlanType=M&IsEvent=Y&IsClose=',@ORDER_BY=0,@TOTAL_COUNT=@p5 output        
select @p5        
...
---------------------------------------------------------------------------------------------------        
 2021-09-14  제휴(ETBS)기획리스트 국내제외 AND A.SIGN_CODE <> 'D'
-------------------------------------------------------------------------------------------------*/         
CREATE PROCEDURE [dbo].[ZP_ETBS_WEB_EVT_SEARCH_LIST_SELECT] 
...
WHERE 1 = 1 AND ISNULL(A.SIGN_CODE,'''') <> ''D'' ' + @WHERE + 
	    ';        
...
(        
 ...
 WHERE  ISNULL(A.SIGN_CODE,'''') <> ''D'' ' + @WHERE + '        
