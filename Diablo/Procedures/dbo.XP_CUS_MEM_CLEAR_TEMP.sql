...
------------------------------------------------------------------------------------------------------------------
	...
	2021-08-19		김성호			쿼리정리(미사용컬럼제외, view_member적용, 프로세스 정리
================================================================================================================*/ 
CREATE PROCEDURE [dbo].[XP_CUS_MEM_CLEAR_TEMP]
...
BEGIN
	...
	DECLARE @FROM_CUS_NO INT, @CUS_CLEAR_SEQ INT, @TOTAL_CUS_LIST VARCHAR(1050) = CONVERT(VARCHAR(10), @CUS_NO) + ',' + @CUS_NO_LIST
	...
	BEGIN
		...
		BEGIN
			...
			FROM (
				SELECT 1 AS [ORDER_KEY], CUS_ID, CUS_PASS FROM VIEW_MEMBER WHERE CUS_NO = @CUS_NO
				UNION
				SELECT 2 AS [ORDER_KEY], CUS_ID, CUS_PASS FROM VIEW_MEMBER WHERE CUS_NO IN (SELECT Data FROM DBO.FN_XML_SPLIT(@CUS_NO_LIST, ','))
			) A
			ORDER BY ORDER_KEY;
...
--END
	...
	-------------------------------------------------------------------------------------------------------------------
	--회원등급은 대상회원중 가능 높은것 -- 21.08.18 주석 고객등급 CUS_VIP_HISTORY 사용
...
--SET @STEP_MSG= '회원 한명씩 처리 커서 시작 '
			...
			BEGIN
				-------------------------------------------------------------------------------------------------------------------
				-- 21.08.18 주석 최초 가입일 업데이트 상단 구현
				--DECLARE @OLD_CUS_ID VARCHAR(20), @OLD_CUS_ID_MEM VARCHAR(20) --카카오싱크 중복가입자 가입시기존 회원가입일로 변경
				--SELECT
				--	@OLD_CUS_ID_MEM = ISNULL((SELECT CUS_ID FROM VIEW_MEMBER WITH(NOLOCK) WHERE CUS_NO = @FROM_CUS_NO), ''),
				--	@OLD_CUS_ID = ISNULL((SELECT CUS_ID FROM CUS_CUSTOMER_damo WITH(NOLOCK) WHERE CUS_NO = @FROM_CUS_NO),'')
				...
				BEGIN
					...
					BEGIN
						...
						ELSE 
						BEGIN
							SET @POINT_REMARK = '고객병합탈퇴소멸('+ CONVERT(VARCHAR,@CUS_NO) + ')으로병합';
							--탈퇴 소멸 처리로직 
