...
■ EXEC						: 	
	...
	EXEC DBO.SP_ROAD_ADDRESS_SELECT_LIST 1, '', '', '염창동', '292-1', '' -- 지역선택 + 동 + 지번 검색
...
------------------------------------------------------------------------------------------------------------------  
   ...
   2017-01-24		정지용			검색 부분 수정 
================================================================================================================*/ 
CREATE PROC [dbo].[SP_ROAD_ADDRESS_SELECT_LIST]
 	@SEARCH_TYPE INT,
	@SIDO VARCHAR(40),
	@SIGUNGU VARCHAR(40),
...
BEGIN
	...
	BEGIN
		...
		SET @WHERE = @WHERE + ' AND CONTAINS (D.시군구건물명, @BUILD_NAME) ';		
