...
-- ========================================================================= */ 
CREATE PROCEDURE [dbo].[SP_FAX_MASTER_SEARCH_COUNT_TEST]
...
--DECLARE @FAX_TABLE TABLE (
--	FAX_NUMBER1 VARCHAR(4),
--	FAX_NUMBER2 VARCHAR(4),
--	FAX_NUMBER3 VARCHAR(4)
--	)
	
--	INSERT INTO @FAX_TABLE
--	SELECT FAX_NUMBER1 ,FAX_NUMBER2 ,FAX_NUMBER3 
--			FROM EMP_MASTER WITH(NOLOCK)
--			WHERE ( ( @SEARCH_TYPE = 0 AND WORK_TYPE = 1 ) 
--				OR ( @SEARCH_TYPE = 1 )  ) 
--			AND TEAM_CODE = @TEAM_CODE 
--			AND (ISNULL(@EMP_CODE,'')='' OR EMP_CODE =@EMP_CODE)  
--			GROUP BY FAX_NUMBER1 ,FAX_NUMBER2 ,FAX_NUMBER3 
...
--		INNER JOIN FAX_RECEIVE B WITH(NOLOCK)
--			ON A.FAX_SEQ = B.FAX_SEQ 
--		INNER JOIN @FAX_TABLE C 
--			ON B.RCV_NUMBER1 = C.FAX_NUMBER1
--			AND B.RCV_NUMBER2 = C.FAX_NUMBER2
--			AND B.RCV_NUMBER3 = C.FAX_NUMBER3
--	WHERE A.FAX_TYPE = 2 AND A.DEL_YN='N' 
--	AND ( @FAX_CAT_SEQ  = 0 OR A.FAX_CAT_SEQ = @FAX_CAT_SEQ )
--	AND (ISNULL(@START_DATE,'') = '' OR CONVERT(CHAR(10),A.NEW_DATE,121)>=@START_DATE) 
--	AND (ISNULL(@END_DATE,'')='' OR CONVERT(CHAR(10),A.NEW_DATE,121)<=@END_DATE) 
--	AND (ISNULL(@SUBJECT,'')='' OR A.[SUBJECT] LIKE '%'+@SUBJECT+'%' ) 
