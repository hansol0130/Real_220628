...
GO
/*================================================================================================================          
■ SP_NAME			: [FN_SLIME_WEB_PKG_MASTER_SELECT_WHERE]          
■ DESCRIPTION		: LIST 조회시 WHERE 절을 통합관리하기 위한 SP          
■ INPUT PARAMETER	: 
■ EXEC				:           
    
    -- exec DBO.SP_SLIME_WEB_PKG_MASTER_SELECT_WHERE('');
...
================================================================================================================*/
CREATE PROCEDURE [dbo].[SP_SLIME_WEB_PKG_MASTER_SELECT_WHERE] 
(
	@KEY NVARCHAR(MAX) ,
	@WHERE_OUTPUT NVARCHAR(MAX) OUTPUT
)
AS
BEGIN
	...
	BEGIN
	    ...
	    END
	    ELSE IF PATINDEX('%' + @KEYWORD + '%', '제주,제주도,울릉도,국내') > 0
		BEGIN
	...
	BEGIN
	    ...
	    DECLARE @PKG_DETAIL_MASTER_QUERY NVARCHAR(MAX);    
	    DECLARE @DETAIL_FILTERED_MASTERCODES NVARCHAR(MAX);    
	    SET @PKG_DETAIL_MASTER_QUERY = N'    
  SET @DETAIL_FILTERED_MASTERCODES =    
  STUFF((SELECT '','' + CONCAT('''''''',MASTER_CODE,'''''''') AS ''data()''    
  FROM PKG_DETAIL AS AA    WITH(NOLOCK)
  WHERE 1=1    
  ' + @TEMP + N'    
  AND SHOW_YN = ''Y''
  ...
  '; 
	    --PRINT @TEMP
	    EXEC SP_EXECUTESQL @PKG_DETAIL_MASTER_QUERY
	        ,N'@DETAIL_FILTERED_MASTERCODES nvarchar(max) OUTPUT'
	        ,@DETAIL_FILTERED_MASTERCODES = @DETAIL_FILTERED_MASTERCODES OUTPUT 
	    ...
	    SET @WHERE = @WHERE + ' AND A.MASTER_CODE IN (' + @DETAIL_FILTERED_MASTERCODES + ')'
	END
	ELSE
	BEGIN
