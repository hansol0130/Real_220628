...
GO
/*================================================================================================================  
■ USP_NAME     : ZP_WEB_RES_MASTER_INFO_SELECT  
■ DESCRIPTION    : 예약된 상품 정보 OR 상품정보 보기   
■ INPUT PARAMETER   :  
■ OUTPUT PARAMETER   :   
■ EXEC      : ZP_WEB_RES_MASTER_INFO_SELECT 'RP2011221529'  
...
------------------------------------------------------------------------------------------------------------------  
	...
	2019-11-20	안병호   여행자 계약서 작성 날자 항목 추가
	2020-02-18	임원묵   XP_WEB_RES_MASTER_INFO_SELECT -> ZP_WEB_RES_MASTER_INFO_SELECT 로 복사 생성
	2020-11-22	김성호	 대표지역 정보 검색 추가 (희망여행 이벤트 목적)
	2020-12-07	홍종우	 패키지(@PRO_TYPE = 1) ATT_CDOE 컬럼 추가 (MULTI PRICE일때 예약상세 페이지 결제방법 포인트 제거를 위해서 추가)
	2021-05-07	홍종우	 패키지(@PRO_TYPE = 1) INF_FILE_MASTER 테이블 조인조건 일부 제거(PKG_MASTER.SHOW_YN = 'Y') - 모바일 트레블가이드 이미지 보이지 않는 현상 해결
================================================================================================================*/   
CREATE PROC [dbo].[ZP_WEB_RES_MASTER_INFO_SELECT]  
	@RES_CODE RES_CODE   
AS   
...
SELECT @PRO_TYPE =PRO_TYPE , @PROVIDER = PROVIDER , @RES_STATE = RES_STATE FROM RES_MASTER_damo WITH(NOLOCK) WHERE RES_CODE = @RES_CODE   
  
--신한,롯데  , BTMS 복지몰 접수상태일때만   
IF( @PROVIDER IN ( 15 , 17 )  OR  (@PROVIDER = 35 AND @RES_STATE = 0  )  )  
BEGIN  
	SELECT @EMP_CODE = SALE_EMP_CODE FROM RES_MASTER_damo WITH(NOLOCK) WHERE RES_CODE = @RES_CODE   --접수자   
END   
ELSE  
BEGIN  
	SELECT @EMP_CODE = NEW_CODE FROM RES_MASTER_damo WITH(NOLOCK) WHERE RES_CODE = @RES_CODE -- 담당자   
END   
...
BEGIN  
	SELECT   
		...
		D.DEP_TRANS_CODE ,( SELECT KOR_NAME FROM PUB_AIRLINE WITH(NOLOCK) WHERE AIRLINE_CODE = D.DEP_TRANS_CODE ) AS DEP_TRANS_NAME,  D.DEP_TRANS_NUMBER ,  
		D.ARR_TRANS_CODE ,( SELECT KOR_NAME FROM PUB_AIRLINE WITH(NOLOCK) WHERE AIRLINE_CODE = D.ARR_TRANS_CODE ) AS ARR_TRANS_NAME,  D.ARR_TRANS_NUMBER ,  
  
		(SELECT KOR_NAME FROM PUB_CITY WITH(NOLOCK) WHERE CITY_CODE = ( SELECT CITY_CODE FROM PUB_AIRPORT WITH(NOLOCK) WHERE AIRPORT_CODE = D.DEP_DEP_AIRPORT_CODE )) AS DEP_DEP_CITY_NAME,  
		(SELECT KOR_NAME FROM PUB_CITY WITH(NOLOCK) WHERE CITY_CODE = ( SELECT CITY_CODE FROM PUB_AIRPORT WITH(NOLOCK) WHERE AIRPORT_CODE = D.DEP_ARR_AIRPORT_CODE )) AS DEP_ARR_CITY_NAME,  
		(SELECT KOR_NAME FROM PUB_CITY WITH(NOLOCK) WHERE CITY_CODE = ( SELECT CITY_CODE FROM PUB_AIRPORT WITH(NOLOCK) WHERE AIRPORT_CODE = D.ARR_DEP_AIRPORT_CODE )) AS ARR_DEP_CITY_NAME,  
		(SELECT KOR_NAME FROM PUB_CITY WITH(NOLOCK) WHERE CITY_CODE = ( SELECT CITY_CODE FROM PUB_AIRPORT WITH(NOLOCK) WHERE AIRPORT_CODE = D.ARR_ARR_AIRPORT_CODE )) AS ARR_ARR_CITY_NAME,  
  
		B.TOUR_DAY , B.TOUR_NIGHT, B.SEAT_CODE,   
		F.REGION_CODE, F.NATION_CODE, F.STATE_CODE, F.CITY_CODE, F.FILE_NAME_S, F.FILE_NAME_M, 
		F.FILE_TYPE ,   
	...
	LEFT JOIN EMP_MASTER C WITH(NOLOCK) ON C.EMP_CODE = @EMP_CODE   
	LEFT JOIN PRO_TRANS_SEAT D WITH(NOLOCK) ON B.SEAT_CODE = D.SEAT_CODE  
	LEFT JOIN PKG_MASTER E WITH(NOLOCK) ON B.MASTER_CODE = E.MASTER_CODE  
	LEFT JOIN INF_FILE_MASTER F WITH(NOLOCK) ON E.MAIN_FILE_CODE = F.FILE_CODE -- AND E.SHOW_YN = 'Y'  
	LEFT JOIN RES_CONTRACT H WITH(NOLOCK) ON A.RES_CODE = H.RES_CODE   
...
BEGIN  
	...
	LEFT JOIN RES_CONTRACT H WITH(NOLOCK) ON A.RES_CODE = H.RES_CODE AND H.CONTRACT_NO = (SELECT MAX(CONTRACT_NO) FROM RES_CONTRACT WHERE RES_CODE = H.RES_CODE  )  
...
BEGIN  
	...
	BEGIN  
		...
		WHERE A.RES_CODE = @RES_CODE AND A.ROOM_NO = @ROOM_NO   
		) + '<BR>'
