...
■ EXEC						:
 ...
 [XP_NAVER_PKG_MASTER_DATA_STATUS_SELECT] '2019-12-01' , '2020-06-30' , '613'
...
2019-11-29			박형만		TMP_NAVER_MASTER -> NAVER_PKG_MASTEr_MANAGE 
================================================================================================================*/ 
CREATE  PROC [dbo].[XP_NAVER_PKG_MASTER_DATA_STATUS_SELECT]
	@START_DATE DATETIME,
	@END_DATE DATETIME, 
...
--SET @TEAM_CODE = '542' 
	...
	CASE WHEN ISNULL(SHOW_CNT,0) > 0 THEN CONVERT(DECIMAL(5,1),(1.0 - (CONVERT(DECIMAL,INOUT_NO_CNT)  / CONVERT(DECIMAL,SHOW_CNT) ) ) * 100.0) ELSE 0 END  AS INOUT_PRO_CODE_PER ,
	CASE WHEN ISNULL(SHOW_CNT,0) > 0 THEN CONVERT(DECIMAL(5,1),(1.0 - (CONVERT(DECIMAL,SELLPOINT_NO_CNT)  / CONVERT(DECIMAL,SHOW_CNT) ) )* 100.0) ELSE 0 END AS SELLPOINT_NO_CNT_PER ,
	CASE WHEN ISNULL(SHOW_CNT,0) > 0 THEN CONVERT(DECIMAL(5,1),(1.0 - (CONVERT(DECIMAL,SELLPOINT_EX_CNT)  / CONVERT(DECIMAL,SHOW_CNT) ) )* 100.0) ELSE 0 END AS SELLPOINT_EX_CNT_PER ,
	CASE WHEN ISNULL(SHOW_CNT,0) > 0 THEN CONVERT(DECIMAL(5,1),(1.0 - (CONVERT(DECIMAL,PRO_POINT_NO_CNT)  / CONVERT(DECIMAL,SHOW_CNT) )  )* 100.0) ELSE 0 END AS PRO_POINT_NO_CNT_PER ,
	CASE WHEN ISNULL(SHOW_CNT,0) > 0 THEN CONVERT(DECIMAL(5,1),(1.0 - (CONVERT(DECIMAL,SEAT_TIME_NO_CNT)  / CONVERT(DECIMAL,SHOW_CNT) )  )* 100.0) ELSE 0 END AS SEAT_TIME_NO_CNT_PER ,
	CASE WHEN ISNULL(SHOW_CNT,0) > 0 THEN CONVERT(DECIMAL(5,1),(1.0 - (CONVERT(DECIMAL,SCH_NO_CNT)  / CONVERT(DECIMAL,SHOW_CNT) )  )* 100.0) ELSE 0 END AS SCH_NO_CNT_PER 
	FROM 
	(
		-- 1 행사 입력 정보 
		SELECT 
		...
		( 
			...
			FROM NAVER_PKG_MASTER_MANAGE AA WITH(NOLOCK)
			...
			WHERE 1=1
			AND CC.SHOW_YN ='Y'   -- 노출인것만? 
			AND (AA.DEL_YN IS NULL OR AA.DEL_YN <>'Y' )
			AND (AA.TEAM_CODE = @TEAM_CODE OR ISNULL(@TEAM_CODE ,'') = '' ) 
