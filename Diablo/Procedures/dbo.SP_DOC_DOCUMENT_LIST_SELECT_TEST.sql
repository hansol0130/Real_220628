...
CREATE PROCEDURE [dbo].[SP_DOC_DOCUMENT_LIST_SELECT_TEST]
	@FLAG			VARCHAR(3),
	@EMP_CODE		CHAR(7),
	@PAGESIZE		INT,
	@IPAGE			INT,
...
AS
BEGIN
	DECLARE @QUERY VARCHAR(8000), @GROUP_CODES VARCHAR(100);
	...
	BEGIN
		...
		WHERE EDI_CODE NOT IN (
			...
			WHERE B.APP_TYPE = ''2''
		) AND RCV_TEAM_CODE IN (SELECT TEAM_CODE FROM EMP_MASTER WHERE EMP_CODE = ''' + @EMP_CODE + ''')
		--#검색조건'
	END
	ELSE IF @FLAG = '92'
	BEGIN
	...
	BEGIN
		IF ((SELECT ISNULL(GROUP_CODE, '') FROM EMP_MASTER WHERE EMP_CODE = @EMP_CODE) = '')
		BEGIN
			SET @QUERY = '
			SELECT COUNT(*) FROM EDI_MASTER
			WHERE EDI_STATUS = ''3'' AND RCV_TEAM_CODE IN (
			SELECT TEAM_CODE FROM EMP_MASTER WHERE EMP_CODE = ''' + @EMP_CODE + ''')
			--#검색조건'
		END
		ELSE
		BEGIN
			WITH GROUP_LIST AS
			(
				SELECT GROUP_CODE, PARENT_CODE, 0 AS [DEPTH] FROM GROUP_MASTER
				WHERE GROUP_CODE IN
				(
					SELECT GROUP_CODE FROM EMP_MASTER
					WHERE EMP_CODE = @EMP_CODE
				)
				UNION ALL
				SELECT A.GROUP_CODE, A.PARENT_CODE, B.DEPTH + 1
				FROM GROUP_MASTER A
				INNER JOIN GROUP_LIST B ON B.PARENT_CODE = A.GROUP_CODE
			)
			SELECT
			...
			(
				SELECT TEAM_CODE FROM GROUP_DETAIL
...
--EXEC SP_DOC_DOCUMENT_LIST_SELECT '112', '2005003', 15, 0, ''
GO
