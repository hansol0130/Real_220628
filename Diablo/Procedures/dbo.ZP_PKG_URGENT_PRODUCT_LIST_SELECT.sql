...
GO
/*================================================================================================================    
■ USP_NAME     : ZP_PKG_URGENT_PRODUCT_LIST_SELECT    
...
declare @p5 int    
set @p5=NULL    
exec ZP_PKG_URGENT_PRODUCT_LIST_SELECT @PAGE_INDEX=1,@PAGE_SIZE=10,@KEY=N'SiteCode=VGT&SignCode=&EmpCode=2015006&TeamCode=529&SearchType=ProCode&SearchValue=&Condition=E',@ORDER_BY=1,@TOTAL_COUNT=@p5 output    
select @p5    
■ MEMO      :     
------------------------------------------------------------------------------------------------------------------    
■ CHANGE HISTORY                       
...
------------------------------------------------------------------------------------------------------------------    
   ...
   2020-02-18  임원묵 SP_PKG_URGENT_PRODUCT_LIST_SELECT -> ZP_PKG_URGENT_PRODUCT_LIST_SELECT 로 복사 생성   
================================================================================================================*/     
CREATE PROCEDURE [dbo].[ZP_PKG_URGENT_PRODUCT_LIST_SELECT] 
(@PAGE_INDEX INT ,@PAGE_SIZE INT ,@TOTAL_COUNT INT OUTPUT ,@KEY VARCHAR(500) ,@ORDER_BY INT)
AS
BEGIN
	...
	SELECT @SITE_CODE = DBO.FN_PARAM(@KEY ,'SiteCode')
	...
	ELSE
	BEGIN
	    IF ISNULL(@TEAM_CODE ,'') <> 'all'
	       AND ISNULL(@TEAM_CODE ,'') <> ''
	    BEGIN
	...
	-- SORT 조건 만들기      
	SELECT @SORT_STRING = (CASE @ORDER_BY WHEN 1 THEN 'A.NEW_DATE DESC' WHEN 2 THEN 'F.DEP_DEP_DATE ASC, F.DEP_DEP_TIME ASC' ELSE 'A.NEW_DATE DESC' END)    
