...
------------------------------------------------------------------------------------------------------------------
   2021-12-01		오준혁			최초생성
================================================================================================================*/ 
CREATE PROCEDURE [onetime].[ZP_EVT_LONGTERM_UNUSED_V3]
AS 
BEGIN
	...
	DECLARE @TMPL_CD       VARCHAR(20) = 'SYS_AT_0178'	-- 사용 템플릿 번호
	       ...
	       ,@REQ_DTM       VARCHAR(14) = FORMAT(GETDATE(), 'yyyyMMdd173000') -- 알림톡 발송일시 (금일 오전 10시로 예약)
	...
	FROM   onetime.EVT_LONGTERM_UNUSED3 a
