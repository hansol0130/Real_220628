...
/*================================================================================================================
■ USP_NAME					: XP_UTIL_QUERY_MAKER
■ DESCRIPTION				: 테이블 기본 쿼리 및 클래스 선언문 생성
■ INPUT PARAMETER			: 
	@TableName				: 테이블 명
	@Schema					: 스키마 명 (기본 dbo)
■ OUTPUT PARAMETER			: 
■ EXEC						: 
	...
	exec XP_UTIL_QUERY_MAKER EMP_MASTER
...
------------------------------------------------------------------------------------------------------------------
   2015-01-15		김성호			최초생성
================================================================================================================*/ 
CREATE PROC [dbo].[XP_UTIL_QUERY_MAKER]
(
...
DECLARE @T_MappingType TABLE (Name VARCHAR(30) , NetType VARCHAR(30), AdoType VARCHAR(30))    
	...
	Select 
		...
		+ '	[DatabaseAttribute("' + Sac.name + '")]' + char(13)
		+ '	public ' + NetType + ' ' + (SELECT UPPER(SUBSTRING(DATA, 1, 1)) + LOWER(SUBSTRING(DATA, 2, 100)) FROM FN_SPLIT(Sac.name, '_') FOR XML PATH('')) + ' { get; set; }' + char(13) + char(13)
	From sys.all_columns As Sac
	Inner Join sys.Tables As Stb On Stb.Object_id = Sac.Object_Id
	Left Outer Join (SELECT major_id, minor_id, VALUE FROM sys.extended_properties WHERE class = 1) AS Sep ON Sep.major_id = Sac.object_id AND Sep.minor_id = Sac.column_id
	Left Outer join [INFORMATION_SCHEMA].[COLUMNS] As infoCol On infoCol.Table_Name = Stb.Name And infoCol.Column_Name = Sac.name
	Left Outer Join (
		SELECT COLUMN_NAME 
		FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
		WHERE OBJECTPROPERTY(OBJECT_ID(CONSTRAINT_NAME), 'ISPRIMARYKEY') = 1 AND TABLE_NAME = @TableName
	 ) As infoKcu On infoKcu.COLUMN_NAME = Sac.name
	LEFT OUTER JOIN @T_MappingType AS T_MappingType ON T_MappingType.Name = InfoCol.Data_Type
	Where Stb.Name = @TableName
	...
	Print 'Select ' + @Sql_Select + char(13) + 'From ' + @TableName + Char(13) + 'Where ' + Replace(@Sql_WherePk,'And', Char(13) + ' And')
	Print  char(13) 
	Print 'Select ' + @Sql_Select + char(13) + 'From ' + @TableName + Char(13) + 'Where ' + Replace(@Sql_WhereAll,'And', Char(13) + ' And')
	Print  char(13) 

	--- Delete 관련 쿼리 모음
	--Print 'Delete 관련 쿼리 모음============================='
	--Print 'Delete From ' + @TableName + Char(13) + ' Where ' + Replace(@Sql_WherePk, 'And', Char(13) + ' And')
	--Print  char(13)
	--Print 'Delete From ' + @TableName + Char(13) + ' Where ' + Replace(@Sql_WhereAll, 'And', Char(13) + ' And')
	--Print  char(13)

	--- Update 관련 쿼리 모음
	Print 'Update 관련 쿼리 모음============================='
	Print 'Update ' + @TableName + ' Set ' + Char(13) + ' ' + Replace(@Sql_WhereAll, 'And', Char(13) + ' ,') + char(13) + 'From ' + @TableName + Char(13) + 'Where ' + Replace(@Sql_WherePk,'And', Char(13) + ' And')
	Print  char(13)
	Print 'Update '+@TableName+' Set ' + Char(13) + ' ' + Replace(@Sql_WhereAll,'And',Char(13) + ' ,') + char(13) + 'From ' + @TableName + Char(13) + 'Where ' + Replace(@Sql_WhereAll,'And' , Char(13) + ' And')
	Print  char(13)

	--- Insert 관련 쿼리 모음
	Print 'Insert 관련 쿼리 모음============================='
	Print 'Insert ' + @TableName + ' (' + @Sql_Select + ') ' + Char(13) + ' Values(@' + Replace(@Sql_Select, ',', ',@') + ')'
	Print  char(13)
	...
	--- Class 선언
	Print 'Class 선언============================='
	Print 'public class ' + @TableName + 'RS : BaseRS' + Char(13) + '{' + Char(13) + @Net_Declare + '}'
	Print char(13)
