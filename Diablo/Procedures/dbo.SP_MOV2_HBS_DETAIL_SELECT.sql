...
/*================================================================================================================
■ USP_NAME					: SP_MOV2_HBS_DETAIL_SELECT
■ DESCRIPTION				: 게시물 검색
■ INPUT PARAMETER			: 
	@MASTER_SEQ				: 마스터 순번
	@BOARD_SEQ				: 게시물 순번
...
set statistics profile off
	...
	exec SP_MOV2_HBS_DETAIL_SELECT 4, 35008, 'N'
	exec SP_MOV2_HBS_DETAIL_SELECT 1, 14010, 'N'
...
------------------------------------------------------------------------------------------------------------------
   ...
   2022-05-31       이장훈          질문에 여러개 답변중 가장최근 한개만 보이도록 수정
================================================================================================================*/ 

CREATE  PROCEDURE [dbo].[SP_MOV2_HBS_DETAIL_SELECT]
(
	@MASTER_SEQ	INT,
	@BOARD_SEQ	INT,
	@COUNT_YN	VARCHAR(1)
...
BEGIN
	...
	(
		...
		SELECT MASTER_SEQ, BOARD_SEQ FROM (
				SELECT
					*
					, ROW_NUMBER() OVER (PARTITION BY PARENT_SEQ ORDER BY BOARD_SEQ DESC) AS ROW_NUM
				FROM HBS_DETAIL WHERE MASTER_SEQ = @MASTER_SEQ AND PARENT_SEQ = @BOARD_SEQ
		) T
		WHERE ROW_NUM = 1
	)
	...
	LEFT JOIN EMP_MASTER_DAMO D WITH(NOLOCK) ON C.EMP_CODE = D.EMP_CODE
