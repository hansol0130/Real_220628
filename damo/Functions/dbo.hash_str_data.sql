...
begin
	...
	select @error_result = damo.dbo.throwErrorApi('damo.dbo.hash_str_data', '', '', 'data', 'ENCRYPTION ERROR : OVER 256 CHARACTERS','ENC') 
end
		declare @inBinaryData varbinary(8000) 
		select @inBinaryData = convert(varbinary(8000), @inData) 
		select @out = damo.dbo.hash_str_fast_data3('B64', '3' , @inBinaryData  ) 
	end 
